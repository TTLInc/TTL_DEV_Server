...
class Lessonconvert extends TL_Controller {
	...
	function index(){
		$qry = $this->db->query("select `id`,`source` from `lessons` where `upload_status` = '0' ") or die(mysql_error());
		if($qry->num_rows()>0)
		{
			...
			foreach($result as $key=>$value){
				...
				if(strtolower($ext[1]) != "mp4"){
					$response = $this->convertVideoProcess($value['source']); // pass video file name 
					if($response)
					{
						$mp4video =  explode(".",$value['source']);
						$mp4videoname = $mp4video[0].".mp4";
						$this->db->query("update `lessons` set `source`= '".$mp4videoname."',`upload_status` = '1' where `id` = '".$value['id']."' and `upload_status` = '0'") or die(mysql_error());
					}
				}
				else
				{
					$this->db->query("update `lessons` set `upload_status` = '1' where `id` = '".$value['id']."' and `upload_status` = '0'") or die(mysql_error());
					$this->generate_thumb();
				}
			}
		}
	...
	function generate_thumb(){
		$qry = $this->db->query("select `id`,`source` from `lessons` where `upload_status` = '1'") or die(mysql_error());
		if($qry->num_rows()>0)
		{
			$result = $qry->result_array(); // Get Result in Array
			foreach($result as $key=>$value){
				$response = $this->videothumb($value['source']); // pass video file name 
				
				if($response)
				{
					$this->db->query("update `lessons` set `upload_status` = '2' where `id` = '".$value['id']."' and `upload_status` = '1'") or die(mysql_error());
				}
			}
		}
	}
	...
	{
		$uploaddir	=	FCPATH."uploads/video/";
