    ...
    <?php $j = 0; foreach ($payments as $key => $val) :?>
        <tr id="id_<?php echo $val['id'];?>">
           <!-- <td><input type="checkbox" name="ids[]" value="<?php echo $val['id'];?>" /></td>-->
			...
			<td><?php echo @$val['id'];?></td>
			<td><?php echo @$val['affiname']." (".@$val['email'].")";?></td>
			<td><?php if(@$val['sid'] !=''){echo @$val['sid']; }else{echo '0';}?></td>
			<td><?php if(@$val['amount'] !=''){echo @$val['amount']; }else{echo '0';}?></td>
			<td><?php 
			$lastpaiddatesql = "SELECT paid_date from affiliate where refid = ".$val['id']." order BY id DESC";
			$lsquery = $this->db->query($lastpaiddatesql);
			$lresult = $lsquery->row_array();
			$lastPaidDate = $lresult['paid_date'];
			if($lastPaidDate != ''){ echo $lastPaidDate; }else{ echo '-'; }
			?></td>
            <?php 
            	// hour calculation            	
					$class = "actionenabled";
					if($this->session->userdata('roleId')!=1002)
					{
					?>
					<td id="actions" class="<?php echo $class; ?>" >
						...
						<?php  }else{?>
						<a href="javascript:void(0);" class="appcat" mid="<?php echo $val['id']; ?>" val="<?php echo  $val['amount'] ;//echo $val['approve']; ?>" schlid="<?php echo $val['sid']; ?>" title="Approve"><img alt="Approve" src="<?php echo base_url('/images/approve-red.png');?>" /></a>
						<?php  } ?>
						<?php  if($val['paid'] == 1){?>
...
setTimeout('showmenu("financial",0)',1000);
function del(_id){
...
function approve(_id,_val,_schlid)
{
	if(window.confirm('Are you sure you want to approve this payment?')){
		$.post('<?php echo Base_url("admin/affi_pay_approve")?>',{id:_id,approveval:_val,schlid:_schlid},function(msg){
...
/*$(".submit-search").click(function() {
	...
	$('.appcat').click(function(){
		approve([$(this).attr('mid')],[$(this).attr('val')],[$(this).attr('schlid')]);
	})
})
</script>
