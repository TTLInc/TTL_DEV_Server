...
class Search extends TL_Controller {
	...
	*/
	...
	public function search()
	{
		...
		$data['config'] = $this->location_model->getConfig();
		$data['multi_lang'] = $multi_lang;
		//echo $multi_lang;
		$data['langsAll'] =$this->langs_model->getLangs();
		...
		$data['langsAll2'] = $langsAll2;
		//echo "<pre>";print_r($langsAll2);exit;
	    ...
	    $data['school'] = $this->db->get()->row()->firstName;
		
		//$this->layout->setData('school_name',$school_name);	
		$this->load->view('angular_search/angular_search',$data);
	}
	//=============================ANGULAR JS SEARCH FUNCTION
	...
	public function doSearch_oct_17(){
		...
		if(isset($searchData['results'])){
			...
			foreach($searchData['results']	as $tutor_search_record){
				if(isset($searchData['results'][$tutor_search_records_counter]['school_id'])	&& $searchData['results'][$tutor_search_records_counter]['school_id'] != ''){
					...
					//----RD-- Commented Virang's code JULY 27--------//
					/*
					if ($resFree[0]['is_eligible']!="1") {
						...
						$searchData['results'][$tutor_search_records_counter]['hRate'] 		= '0.00';
						$searchData['results'][$tutor_search_records_counter]['o_hRate'] 	= '0.00';
						$searchData['results'][$tutor_search_records_counter]['o_mark'] 	= '0.00';
					}
					*/
					//----RD-- Commented Virang's code JULY 27--------//
...
//exit;		
		if($_GET['v']){
			...
			if(isset($playlesson['uid'])){
				$tutorprofile = $this->profile_model->getTutorProfile($playlesson['uid']);
			}
		}else{		
			if(isset($lessons[0]) && isset($lessons[0]['uid'])){
				$tutorprofile = $this->profile_model->getTutorProfile($lessons[0]['uid']);
			}
