	...
	**/
	public function getTutorProfile($uid){
	 ...
	 //$sql = "select p.uid,p.firstName,p.lastName,p.hRate,p.pic,p.city,c.country,pr.provice,u.readytotalk from profile AS p JOIN countries as c JOIN user as u JOIN  provices AS pr where p.uid=$uid and u.id=$uid  limit 1";		
		...
		$sql = "SELECT profile.uid,profile.firstName,profile.lastName,profile.school_id,profile.pic,profile.city,countries.country ,provices.provice,user.readytotalk $setPrfHrate from profile LEFT JOIN user ON  user.id=profile.uid  LEFT JOIN countries ON countries.id = profile.country LEFT JOIN provices on provices.id = profile.province WHERE profile.uid={$uid} limit 1";
		$query = $this->db->query($sql,array($uid));
	...
	public function getProfileJoin($uid){
		$result = array();
		$sql = "SELECT profile.*,countries.country as countryName , provices.provice as stateName from profile LEFT JOIN countries ON countries.id = profile.country LEFT JOIN provices on provices.id = profile.province WHERE uid=? limit 1";
        $query = $this->db->query($sql,array($uid));
        if ($query->num_rows() > 0){
            $result = $query->row_array();
        }
	...
	public function getRatings($id,$start=0,$length=8){
		$sql = "SELECT feedback.*,profile.firstName,profile.lastName,profile.pic FROM feedback LEFT JOIN profile ON profile.uid=feedback.receiverId WHERE callerId={$id} AND feedback.status = 1 ORDER BY feedback.id desc limit {$start},{$length}";
		$result = array();
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0){
			$result = $query->result_array();
...
SELECT `id` FROM `user` WHERE `roleId`=4
	...
	public function GroupRating($id,$start=0,$length=8){
		$sql = "SELECT groupfeedback.*,profile.firstName,profile.lastName,profile.pic FROM groupfeedback LEFT JOIN profile ON profile.uid=groupfeedback.receiverId WHERE callerId={$id} ORDER BY groupfeedback.groupFeedback_id desc limit {$start},{$length}";
		$result = array();
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0){
			$result = $query->result_array();
