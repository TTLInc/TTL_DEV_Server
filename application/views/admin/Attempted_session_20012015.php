    ...
    <?php $j = 0; foreach ($payments as $key => $val) :?>
        <tr id="id_<?php echo $val['id'];?>">
           <!-- <td><input type="checkbox" name="ids[]" value="<?php echo $val['id'];?>" /></td>-->
            ...
            <td>
           <?php if(@$val['session_type'] == 'free' && @$val['Intent']==0)
			{
					echo 'Free';
			}
			
			else if(@$val['session_type'] == 'requested' && @$val['Intent']==0)
			{
					echo 'Requested';
			}
			else if(@$val['Intent']>=1)
			{
					echo 'Aborted';
			}
			else
...
//alert(_id);return false;
	if(window.confirm('Are you sure you want to postpone this payment?')){
		$.post('<?php echo Base_url("admin/disputes_postpondsss")?>',{id:_id},function(msg){
...
//alert(_id);return false;
	if(window.confirm('Are you sure you want to approve this payment?')){
		$.post('<?php echo Base_url("admin/disputes_approvesss")?>',{id:_id,approveval:_val},function(msg){
			if (String == msg.constructor) {      
				eval ('var json = ' + msg);
			} else {
				var json = msg;
