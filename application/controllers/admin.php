...
class Admin extends TL_AdminController {
	...
	public function user($sort='id',$sortorder='asc',$page_num=1){
		...
		if($sort != 'completedSessionDate'){
			...
			$sql = "select u.*, CONCAT(p.firstName, ' ',p.lastName) as usernm , p.lms_complete, p.money, p.affiliate_credits, p.coupon_credits, p.purchased_credits, p.earned_credits from user as u,profile as p where u.id = p.uid";
			$query = $this->db->query($sql);
			if ($query->num_rows() > 0) {
	...
	public function user_edit(){
		...
		if($this->input->post()){
			...
			if(isset($data['purchased_credits'])){
				$profileData['purchased_credits'] = $data['purchased_credits'];
			}
			if(isset($data['earned_credits'])){
				$profileData['earned_credits'] = $data['earned_credits'];
			}
			if(isset($data['coupon_credits'])){
				$profileData['coupon_credits'] = $data['coupon_credits'];
			}
			unset($data['money']);
			unset($data['cur_bal']);
			unset($data['expDate']);
			unset($data['purchased_credits']);
			unset($data['earned_credits']);
			unset($data['coupon_credits']);
			//--Update Balance
			/* added by Ilyas */
			if($data['roleId'] <= '1')
			{
			...
			if(count($users)>0)
			{
				if($data['msgtype'] == 'affiliate')
				{
					...
					for($i=0; $i<count($users); $i++){
						...
						$str = "";
						$str = "Dear ".$name."\r\n<br/>";
						$str .= "Affiliate #: http://dev.thetalklist.com/".$users[$i]['id']."\r\n<br/>";
						$str .= $message."\r\n<br/>";
						$str .= "\r\n<br/>";
 ...
 */	
	...
	**/
	...
	public function get_school_payment_note(){
		$this->load->model(array('user_model','profile_model'));
		$id = $this->input->_request('id');
		...
		if($this->input->post()){
			...
			if($errormsg == ""){
				...
				$data['school_id'] = $id;
				$data['date'] = date('Y-m-d H:i:s',time());
				$this->user_model->school_payment_addnote($data);
				$errormsg = 'Payment note has been added successfully.';
			}
		}
		...
		$usernotes = $this->user_model->GetAllSchoolPaymentNotes($start,$sort,$sortorder);

		$this->load->library('paginationadmin');
		$config['base_url'] = base_url('admin/school_payment_note/start');
		$config['uri_segment'] = 5;		
		if($search != ''){
			//$config['total_rows'] = $this->user_model->getNoteCountBySearch($start,$id,$sort,$sortorder);
			$config['total_rows'] = $this->user_model->getSchoolPaymentNoteCount();
		}else{
			$config['total_rows'] = $this->user_model->getSchoolPaymentNoteCount();
		}
		...
		//$note_data = $this->user_model->getUserNote($id);
		$this->layout->setData('usernotes',$usernotes);
		$this->layout->view('admin/school_payment_note');
	}
	...
	public function schoolsummary(){
		...
		$targetpage = base_url()."admin/schoolsummary/sortorder/".$sortorder."/sort/".$sort."/limit/".$limit;
		$page = $this->input->_request('page');
		if($page)
			$start = ($page - 1) * $limit;
		else
		...
		$this->layout->setData('payments',$payments);
		$this->layout->setData('start',$start);
		$this->layout->view('admin/schoolsummary');
	}
	...
	public function schoolpayment(){
		...
		if($search !='')
		{
			$paymentsAll = $this->pay_model->GetSchoolPaymentForSearch($start,$search,$limit,$sortorder,$sort);
		}
		else
		{
			$paymentsAll = $this->pay_model->getAllSchoolPaymentTransaction($start,$dat,$limit,$sortorder,$sort);
		}
		if($paymentsAll=='')
		{
			$start = 0;
		...
		if($paymentsAll)
		{
			...
			foreach ($paymentsAll as $payf)
			{
				$payments[$i]['id'] = @$payf['id'];
				$payments[$i]['school_id'] = @$payf['school_id'];
				$payments[$i]['school_name'] = @$payf['school_name'];
				$payments[$i]['t_payment'] = @$payf['t_payment'];
				$payments[$i]['s_payment'] = @$payf['s_payment'];
				$payments[$i]['date_earned'] = @$payf['date_earned'];
				$payments[$i]['date_paid'] = @$payf['date_paid'];
				$i++;
			}
		}
		$this->layout->setData('sortorder',$sortorder);
	...
	public function a_summary(){
			...
			if($affi)
		{
			...
			foreach ($affi as $payf)
			{
				...
				if($payf['organizationName'] != ''){
					$payments[$i]['affiname'] = $payf['firstName']. $payf['lastName'].'-'.$payf['organizationName'];
				}else{
...
//exit;
	...
	public function  affi_pay_approve(){
		...
		$schlid = $this->input->_request('schlid');
		...
		if(isset($massPayResponse)) {
			if($massPayResponse->Ack == 'Success'){
				...
				//---- Insert affiliate note start-------------------
				$paiddate = date('Y-m-d H:i:s',time());
				$data['role'] = '1000';
				$data['dispute_id'] = '0';
				$data['note'] = $val[0]." | ".$schlid[0]." | ".$paiddate;
				$data['school_id'] = $rid[0];
				$data['date'] = date('Y-m-d H:i:s',time());
				$this->user_model->Affi_addnote($data);
				//---- Insert affiliate note end-------------------
...
/*echo $this->uri->segment(4);
	...
	public function AproveSchoolPayment()
	{
		...
		$schoolname = $this->input->_request('schoolname');
		//$schoolid = $this->input->_request('schoolid');
		$tpayment = $this->input->_request('tpayment');
		$spayment = $this->input->_request('spayment');
		$datepaid = date('Y-m-d H:i:s');

		$sql = "insert into school_payment_transaction SET school_id = '{$schoolid[0]}', school_name = '{$schoolname[0]}', t_payment = '{$tpayment[0]}', s_payment = '{$spayment[0]}', date_paid = '{$datepaid}'";
		$query11 = $this->db->query($sql);
...
function TimeGmt($time,$localTimeZone='a'){
	...
	public function school_payment_transaction_del()
	{
		$id = $this->input->_request('id');
		$this->load->model(array('pay_model'));
		$this->pay_model->del_school_payment_transaction($id);
		echo json_encode(array('status'=>true,'ids'=>$id));
	}

	public function displayhomevideo()
	{
		$this->load->model(array('lesson_model'));
		$id = $this->input->post('id');
		$topvideo = $this->lesson_model->getHomeVideo();
		//print_r($topvideo);
		if($topvideo[0]['id'] != $id){
			if(!empty($topvideo)){					
				$message = '1';
			}else{
				$this->lesson_model->chnghomevideostatus($id);
			}
		}else{
			$topcount = count($topvideo);
			if ($topcount > 0 AND $topvideo[0]['id'] == $id) {
				$this->lesson_model->chnghomevideostatus($id);
			}
		}
	...
	public function displaydashvideo()
	{
		$this->load->model(array('lesson_model'));
		$id = $this->input->post('id');
		//$topvideo = $this->lesson_model->getHomeVideo();
		...
		if ($id) {
			echo $this->lesson_model->chngdashvideostatus($id);
		}
	}

	public function play_video_view(){
		
		$data = array();
		$urluidnew = $this->input->_request('vid');
		if($this->uri->segment(4) != '' || $urluidnew != ''){
			$fsql = "SELECT * FROM lessons where id = ".$urluidnew;
			$fquery = $this->db->query($fsql);
			$fresult = $fquery->row_array();
			$data['fresult'] = $fresult;
		}
		if(urldecode($this->input->_request('vid'))){
			$uidurl = urldecode($this->input->_request('vid'));
		}else{
			$uidurl = 0;
		}
		$data['uidurl'] = $uidurl;
		$this->load->view("admin/play_video_view",$data);
	}
	
	public function school_credit() {
	// echo "hi";die();
		...
		$this->load->view("admin/school_credit",array('data'=>$result));
	}
	...
	public function enter_credit(){
		 ...
		 $date = date('Y-m-d');
			...
			if($credit != ''){
				$sql = "INSERT INTO organization_payment (school_id,description,credit_balance,date) VALUES ( '{$school_id}','Credit Purchase','{$credit}','{$date}')";
				$query = $this->db->query($sql);
			}
			if($platform != ''){
				$sql = "INSERT INTO organization_payment (school_id,description,credit_balance,date) VALUES ( '{$school_id}','Platform fee','{$platform}','{$date}')";
				$query = $this->db->query($sql);
			}
			
			$sql_update = "UPDATE user set credit_balance = credit_balance + '{$credit}' WHERE id = '{$school_id}' ";
			$query_update = $this->db->query($sql_update);
			
			$sql_proupdate = "UPDATE profile set money = money + '{$credit}' WHERE uid = '{$school_id}' ";
			$query_proupdate = $this->db->query($sql_proupdate);
			...
			if($query_update){
				$query = $this->db->select('*')
								...
								->get();
				$result = $query->result();
				$this->session->set_flashdata('msg', 'Credit Added Succesfully');
				return redirect('admin/school_credits',array('data'=>$result));
			}else{
				...
				$query = $this->db->select('*')
								...
								->get();
				$result = $query->result();
				redirect('admin/school_credits',array('data'=>$result));
	...
	public function school_credits(){
		$query = $this->db->select('*')
								->from('profile')
								->join('user', 'profile.uid = user.id', 'inner')
								->where('user.roleId', 4)
		...
		$this->layout->view('admin/school_credits',array('data'=>$result));
	}
	...
	public function rewards(){
		$query = $this->db->select('*')
								...
								->get();
				$result = $query->result();
		$this->layout->view('admin/rewards',array('data'=>$result));
	}
	
	public function rewa_del(){
		$id = $this->input->_request('id');
		$this->db->where_in('id', $id);
		$this->db->delete('rewards_table');
		echo json_encode(array('status'=>true,'ids'=>$id));
	}
	...
	public function list_schools(){
		...
		$this->load->library('pagination');
		$this->db->where(array('roleId'=>4,'credit_balance'=>''));
		$this->db->from('user');
		$count_balance = $this->db->count_all_results();
		
		$total_row = ($count_user - $count_balance); 
		
		$config = array();
		$config['base_url'] = 'http://dev.thetalklist.com/admin/list_schools';
		...
		//$this->layout->setData('links',$links);
		$this->layout->view('admin/list_schools',array('data'=>$result));
	}
}
