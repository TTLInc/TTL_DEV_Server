...
<div class="how_works">
	...
	<li class="nav_sel"><a href="<?php echo base_url('/support/resources');?>"><?php if($this->session->userdata['roleId'] == 0){ echo 'Student ';}else { echo 'Tutor ';}?>Resources</a></li>
	 ...
	 <div class="faqs-ttl content" style="font-size:12px;">
		<h2>
		<?php if($this->session->userdata('roleId') == 0 || (($this->uri->segment(3)=='student') and ($this->session->userdata('universal_roleId')==1)) ): ?>
	...
	<div class="how_works_mid">
		<p style="font-size:14px;text-align:center;">Select and play your training video.</p>
		<div class="mgauto paly_box"><div class="video_Wp posR "  id="player_b"><div class="projekktor" id="player_a"></div></div></div>
			
		<h1 class="nowTitle" style="text-align:center;"></h1>
		...
		<?php $j = 0;foreach($resources as $k=>$resource):?> 
			...
			<?php if($resource['year'] != '0'){ ?>
			<dl source="<?php echo base_url('/vedio/support/'.$resource['year'].'/0'.$resource['month'].'/'.$resource['date'].'/'.$resource['vfile']);?>" class="showVideo <?php if($j%2 == 1){echo "even";}$j++;?> ">
			<?php } else { ?>
			<dl source="<?php echo base_url('/vedio/support/'.$resource['vfile']);?>" class="showVideo <?php if($j%2 == 1){echo "even";}$j++;?> ">
			<?php } ?>
			<dt class="play_box_small">
				<span href="#">
				<?php //echo "showVideoeven7".$j++;?>
					<span class="layer"></span>
					<span class="icon_play"></span>
					<?php if($resource['year'] != '0'){ ?>
					<img class="img" src="<?php echo 'https://www.thetalklist.com/vedio/support/images/'.$resource['year'].'/0'.$resource['month'].'/'.$resource['date'].'/'.$resource['vfile'];?>.jpg">
					<?php } else { ?>
					<img class="img" src="<?php echo 'https://www.thetalklist.com/vedio/support/images/'.$resource['vfile'];?>.jpg">
					<?php } ?>
				</span>
			</dt>
			...
			<p href="#">
				<span class="layer"></span>
				<img src="<?php echo base_url('/uploads/video/images/pdf.jpg');?>" class="img">
			</p>
			</dt>
			<dd>
	...
	$(function(){
		...
		if( typeof(_firstLesson.get(0)) == 'undefined' || _firstLesson.attr('source')==''){
			createVideo('<?php echo base_url('vedio/support/sample_video.mp4.jpg');?>','<?php echo base_url('vedio/support/market_yourself.mp4');?>','Default vedio');
