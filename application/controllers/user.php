...
class User extends TL_Controller {
	...
	public function __construct() {
		...
		parent::__construct();
		session_start();
		$this->load->model(array('user_model','profile_model','langs_model','lesson_model'));
	...
	private function getViewTemp($uid,$temp){		
		...
		if($temp == 'user/lessons' AND $permisson == 'student_private'){
			$permisson = 'teacher_private';
		}
	...
	public function getPermission($uid){
		...
		$school_id = $this->db->get()->row()->school_id;
		
		$this->db->select('school_id');
		$this->db->from('profile');
		$this->db->where('uid',$uid);
		$school_id_user = $this->db->get()->row()->school_id;
			
		$this->layout->setData('school_id',$school_id);
		$this->layout->setData('school_id_user',$school_id_user);
	...
	public function dashboard(){
		...
		$pic = $this->db->get()->row()->pic;

		$this->db->select('*');
		$this->db->from('organization_payment');
		$this->db->where('school_id', $this->user['uid']);
		$organization = $this->db->get();
		$orga_query = $organization->result();
		
		$this->db->select('*');
		$this->db->from('class');
		$this->db->join('profile','profile.uid = class.sid', 'inner');
		$this->db->join('countries',' profile.country = countries.id', 'inner');
		$this->db->where('class.school_id', $this->user['uid']);
		$session = $this->db->get();
		$session_query = $session->result();
		
		$this->db->select_sum('credit_balance');
		$this->db->from('organization_payment');
		$this->db->where('school_id', $this->user['uid']);
		$query = $this->db->get();
		$sum =  $query->row()->credit_balance;
		
		$this->db->select_sum('fee');
		$this->db->from('class');
		$this->db->where('school_id', $this->user['uid']);
		$fees = $this->db->get();
		$total_session =  $fees->row()->fee;
	
		$this->layout->setData('sum',$sum);	
		$this->layout->setData(array('organization'=>$orga_query));	
		$this->layout->setData(array('session'=>$session_query));
		$this->layout->setData('total_session',$total_session);	
		$this->layout->setData('pic',$pic);		//$this->load->model(array('class_model','location_model','ad_model','dashboardmessages_model','dchatmodel','event_model'));
...
/*
		...
		*/
		...
		// Get Category
		$categories = $this->category_model->GetCategories(0,30,"asc","category");
		...
		// Get last video container 6
		$lastvideo = $this->lesson_model->getLastVideo();
		$this->layout->setData('lastvideo',$lastvideo);
...
public function chkNextClsTutorAjax(){
		...
		if(isset($data) && $data !='')
		{
			...
			foreach($data as $msg)
			{
				...
				if($msg['user_id'] != '')
				{
					...
					$userDisplayName = ucfirst(@$profile['firstName']).', '.$cname;
				}else
				{
					$msg['pic'] = base_url().'images/base/hd-pic.png';
				}
	...
	**/
	...
	{
		$this->user_model->user_edit(array('fbshare'=>$this->input->post('fbshare')), $this->uid);
		echo json_encode(array('status'=>'success'));
	}
	/**
	...
	if($this->uid == '')
		...
		if(strpos($permisson,'student') > -1){
			if($this->uid == $this->user['uid']){
				...
				redirect('user/dashboard/uid/'.$this->uid);
			}
			else {
			...
			$resFreeDetails = $this->user_model->fnGetUser("is_eligible",array("id"=>$this->session->userdata('uid')));
			if ($resFreeDetails[0]['is_eligible'] != "1") {
				//$this->layout->setData('schtut',$schoolTut['school_id']);
				$this->layout->setData('schtut','0');
			}else{
				$this->layout->setData('schtut','0');
			}
			//-----RD----//
                        ...
                        //echo $this->uid." : ".$uri." : ".$this->session->userdata('uid');
						  if($this->session->userdata('uid') =='')
						  {
							  $q = "Select hRate,school_id from profile where uid = ".$this->uid;
								...
								$q= "SELECT tutor_markup from profile where uid=".$school_id;
								$classquery = $this->db->query($q);
								$classresult = $classquery->row_array();
								$classresult[0]['tutor_markup'];
		...
		if ($this->input->post()) {
			...
			if (preg_match("/([.]+)/", $formData['refid'], $matches)) {
				...
				$refid  = $decode;
				 ...
				 $sc = $this->user_model->getAffiScid($formData['refid']);
				
				 $school_id = $sc['school_id'];
				 $affiliate_id = $sc['affiliate_id'];
				 $credit_balance = $sc['credit_value'];
			...
			if ($uid) {
				...
				if($refdata['roleId']==4){
					$formData['country'] = $formData['countryId'];	
				...
				if (($formData['roleId']  == 0) and ($formData['universal_roleId'] = 0)) {
					...
					$str .= "<ul>";
					$str .= "<li>Login and view your custom affiliate URL link that you can share with anyone to earn 10% of their first credit purchase.</li>";
					$str .= "<li>If you are connected with a US university, send them an email with your user ID so they can offer you promotion links with credits.</li>";
					...
					$str .= "<li>Get your network to enroll (be sure they use Chrome for live video connections as IE is not supported)</li>";
					$str .= "<li>Monitor transactions and earnings through your account page.</li>";
					$str .= "<li>Get paid to your PayPal account on a monthly basis.</li>";
					$str .= "</ul>";
					
					$str .= "Your account information:\r\n<br/>";
					$str .= "User ID: {$formData['firstName']} {$formData['organizationName']} {$uid}\r\n<br/>";
					$str .= "Email: {$formData['email']}\r\n<br/>";
				}
				...
				if($formData['roleId'] == 0){
					$str_data .="<table border='0' cellpadding='0' cellspacing='0' width='100%'> <tr>
								  ...
								  <table border='0' cellpadding='0' cellspacing='0' width='100%' style='max-width: 500px;' class='responsive-table'>
									...
									<td>
										<table width='100%' border='0' cellspacing='0' cellpadding='0'>
											<tr>
												<td align='center' style='font-size:25px; font-family: Helvetica, Arial, sans-serif; color: #333333; padding-bottom:5px;' class='padding'>Did you know?</td>
											</tr>
											<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: #666666; padding-bottom:30px;' class='padding'>You are part of a worldwide education community?</td>
											</tr>
											<tr>
												<td class='padding' align='center'>
													<img src='http://i.imgur.com/TPJvMnm.jpg' width='500' height='400' border='0' style='display: block; padding: 0; color: #666666; text-decoration: none; font-family: Helvetica, arial, sans-serif; font-size: 16px;' class='img-max'>
												</td>
											</tr>
											<tr>
											<td>
											...
											<table width='100%' border='0' cellspacing='0' cellpadding='0'>
												<tr>
												<td align='center' style='font-size: 20px; font-family: Helvetica, Arial, sans-serif; color: #333333; padding-bottom:5px;' class='padding'>Benefits:</td>
												</tr>
												<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>1. Help how language is shaped worldwide</td>
												</tr>
												</table>
												</td>
												</tr>
												...
												<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>2. Enhance your portfolio in education and language</td>
												</tr>
												</table>
												</td>
												</tr>

												<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>3. 1st, 2nd, and 3rd prizes for most sessions completed by June 30th
												</td>
												</tr>
												<tr>
												<td>
												...
												<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>Watch our orientation video!</td>
												</tr>
												<tr>
												<td align='center'>
												<table width='100%' border='0' cellspacing='0' cellpadding='0'>
												...
												<tr>
												<td align='center' style='border-radius: 3px;' bgcolor='#E46909'><a href='https://www.youtube.com/watch?v=F1fwfaK2pTg&feature=youtu.be' target='_blank' style='font-size: 16px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; border-radius: 3px; padding: 10px 20px; border: 1px solid #E46909; display: inline-block;' class='mobile-button'>Watch Now </a></td>
												</tr>
												</table>
												</td>
												</tr>
									...
									<td>
										<table width='100%' border='0' cellspacing='0' cellpadding='0'>
											...
											<table width='100%' border='0' cellspacing='0' cellpadding='0'>
												...
												<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>3. First to establish profile with potential income
												</td>
												</tr>
												<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>4. 1st, 2nd, and 3rd prizes for most sessions completed by June 30th
												</td>
												</tr>
												<tr>
												<td>
												...
												<tr>
												<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>Complete your profile here</td>
												</tr>
												<tr>
													<td align='center'>
													<table width='100%' border='0' cellspacing='0' cellpadding='0'>
													...
													<table border='0' cellspacing='0' cellpadding='0' class='mobile-button-container'>
													<tr>
													<td align='center' style='border-radius: 3px;' bgcolor='#E46909'><a href='https://www.thetalklist.com/' target='_blank' style='font-size: 16px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; border-radius: 3px; padding: 10px 20px; border: 1px solid #E46909; display: inline-block;' class='mobile-button'>Complete Here </a></td>
													</tr>
													</table>
													</td>
													</tr>
				...
				if($formData['roleId'] == 5){
					$str_data = "<table border='0' cellpadding='0' cellspacing='0' width='100%'>
									<tr>
									<td align='center' style='padding: 70px 15px 70px 15px;' class='section-padding'>
									...
									<tr>
									<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>1. Increase conversion to students abroad.</td>
									</tr>
									</table>
									</td>
									</tr>
									...
									<tr>
									<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>2. Improve attraction and retention of students to US schools.</td>
									</tr>
									</table>
									</td>
									</tr>
									...
									<tr>
									<td align='center' style='padding: 15px 0 0 0; font-size: 14px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>3. Earn money when students purchase credits.
									</td>
									</tr>
									<tr>
									<td>
									...
									<table width='100%' border='0' cellspacing='0' cellpadding='0'>
									<tr>
									<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>Create your account here:</td>
									</tr>
									</table>
									</td>
									</tr>
									...
									<table border='0' cellspacing='0' cellpadding='0' class='mobile-button-container'>
									<tr>
									<td align='center' style='border-radius: 3px;' bgcolor='#E46909'><a href='https://www.thetalklist.com/en/affiliate/index' target='_blank' style='font-size: 16px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; border-radius: 3px; padding: 10px 20px; border: 1px solid #E46909; display: inline-block;' class='mobile-button'>Create Account </a></td>
									</tr>
									</table>
									</td>
									</tr>
									...
									<tr>
									<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>Then start promoting conversations!</td>
									</tr>
									</table>
									<br/>
									<br/>";
				...
				$this->email->to($formData['email']);
				//$this->email->subject('Sign-Up with TheTalkList.com by '.$formData['firstName']);
				$this->email->subject('Welcome To TheTalkList');
				$this->email->message($str_data);
				$this->email->send();
				...
				if($formData['roleId'] == 4){
					...
					if($_SERVER['HTTP_REFERER'] == 'https://www.thetalklist.com/user/signup'){
						redirect(base_url('user/studentcouponPopup'));
					}else{
						if($_GET['uid'] != '0' || $_GET['uid'] != ''){
							
							if($_POST['tutoruserid'] == ''){
								//$sentence = str_replace('https://www.thetalklist.com/en/user/profile/uid/', '', $_SERVER['HTTP_REFERER']);
								//$array = explode("/",$sentence);
								//$tutoruserid = $array[0];

								//$_POST['tutoruserid'] = $tutoruserid;
								$_POST['tutoruserid'] = $uid;
							}
							redirect(base_url('user/studentPopup/?g=tutorpro&uid='.$_POST['tutoruserid']));
						}else{
...
//---R&D-@Oct-19-2013	: Social Promote/** WDC Commit complete*/
	...
	public function login() {		
		...
		$fblogin_url = $this->facebook->getLoginUrl(array('redirect_uri' => site_url('user/fblogin'), 
			'scope' => array("email")));
 ...
 */
	...
	public function export($myval=""){
		...
		$data['Totaltransactions'] = $this->class_model->getMemberTotalTransaction($this->uid, $myval);
	...
	public function teachers(){
		...
		$cdate=date('Y-m-d');
			$free='';	
				if($classresult['exp_session'] > $cdate OR $classresult['is_eligible']==1)
				{
	...
	public function lessons(){
		$current_talk_user_id = $this->session->userdata('uid');
		if($current_talk_user_id == ''){
			redirect('user/login');
		}
	...
	public function lesson_video(){
		...
		//echo "test";
		//print_r($infos['play_time']);
		//exit;
		if($infos['play_time'] > '60.59'){
			//echo "Video is bigger than 1 minute. Please upload video which is less than 1 minute.";
	...
	public function send_message(){
		...
		else
		{
			...
			if($meid != ''){
				...
				$message = "<br/><br/>".$dash."<br/>".$dt."<br/><br/>".$meresult[0]['message'];
			}
...
public function sendBookNowMessage(){
	...
	**/
	public function upload_vee_chat_document()
	{
		...
		if($this->media->error){
			...
			//echo $fileType;exit;
			echo json_encode(array('source'=>'https://www.thetalklist.com/uploads/source/'.$this->media->sqlAddress,'ext'=>$ext,'filetype'=>$fileType));
		}
	}
	...
	**/
	public function checkClassBookNow() {
		...
		$squery = "select school_id, tutor_markup from profile where uid={$tid}";
		$ssql = $this->db->query($squery);
		$csresult = $ssql->row_array();		
		$schoolid = $csresult['school_id'];
...
function output_file($file, $name, $mime_type='')
{
   ...
   function img_download(){
	echo $file_name = "header.jpg";
		 ...
		 public function buyClassesnew()
        {
            ...
            else 
            {
				...
				else
                {
					...
					//----------RD--------------//
					$resFreeDetails 	= $this->user_model->fnGetUser("is_eligible",array("id"=>$this->session->userdata('uid')));
					$iseligibleForFree 	= $resFreeDetails[0]['is_eligible'];
					if($iseligibleForFree == 1){
						$tutorProfile['hRate'] = 0;
					}else{
						$tutorProfile['hRate'] = round($tutorProfile['hRate'] * (1+$config['VEE_PRICE_PERCENT']['value']) *100) /100;
					}
		...
		public function paypalcheck_affi(){
			$checkType = $_GET['name'];
			
				switch($checkType){
					case  'email':$result = $this->user_model->chkpayemail($_GET['name']);break;
					case  'username':$result = $this->user_model->checkUserName($checkType);break;
					default:$result = $this->user_model->checkUserName($checkType);break;
				}
				//$this->output->set_content_type('application/jsonp')->set_output(json_encode($result));
	...
	public function relogin() {
		...
		public function studentcouponPopup()
		{
			$this->layout->view('user/studentcouponpopup');
		}
	...
	public function inCompleteProfileEmail()
	{
		...
		foreach($result as $user){
			...
			if (!in_array($user['email'],$userEmails)) {
				...
				$this->email->mailtype = 'html';
				$this->email->from('no-reply@thetalklist.com','TheTalkList Admin');
				$this->email->subject('Hi there. Your profile on TheTalkList is still incomplete.');
				$this->email->message($str);
				$this->email->to($user['email']);
				...
				$str .="<table border='0' cellpadding='0' cellspacing='0' width='100%'>
							...
							<table width='100%' border='0' cellspacing='0' cellpadding='0'>
							<tr>
							<td align='center' style='font-size: 25px; font-family: Helvetica, Arial, sans-serif; color: #333333; padding-bottom:5px;' class='padding'>Thanks for joining our killer education team!</td>
							</tr>
							<tr>
							<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: #666666; padding-bottom:30px;' class='padding'>Your feedback will change the way the world learns language.</td>
							</tr>
							<tr>
							<td class='padding' align='center'>
							<img src='http://i.imgur.com/TPJvMnm.jpg' width='500' height='400' border='0' style='display: block; padding: 0; color: #666666; text-decoration: none; font-family: Helvetica, arial, sans-serif; font-size: 16px;' class='img-max'>
							...
							<table width='100%' border='0' cellspacing='0' cellpadding='0'>
							<tr>
							<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>1. Complete your profile here</td>
							</tr>
							</table>
							</td>
							</tr>
							...
							<table border='0' cellspacing='0' cellpadding='0' class='mobile-button-container'>
							<tr>
							<td align='center' style='border-radius: 3px;' bgcolor='#E46909'><a href='https://www.thetalklist.com/' target='_blank' style='font-size: 16px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; border-radius: 3px; padding: 10px 20px; border: 1px solid #E46909; display: inline-block;' class='mobile-button'>Complete Here &rarr;</a></td>
							</tr>
							</table>
							</td>
							</tr>
							...
							<tr>
							<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>2. Watch our orientation video</td>
							</tr>
							</table>
							</td>
							</tr>
							...
							<table border='0' cellspacing='0' cellpadding='0' class='mobile-button-container'>
							<tr>
							<td align='center' style='border-radius: 3px;' bgcolor='#E46909'><a href='https://www.youtube.com/watch?v=F1fwfaK2pTg&feature=youtu.be' target='_blank' style='font-size: 16px; font-family: Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; color: #ffffff; text-decoration: none; border-radius: 3px; padding: 10px 20px; border: 1px solid #E46909; display: inline-block;' class='mobile-button'>Watch Now &rarr;</a></td>
							</tr>
							</table>
							</td>
							</tr>
							...
							<tr>
							<td align='center' style='padding: 15px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: black;' class='padding'>3. Login to the community at any of these times:
							<ul style='list-style-type: none; align: center; padding-left:10px;'>
							<li style='margin:5px;'><b>Tuesday </b> 6pm-8pm PST</li>
							<li style='margin:5px;'><b>Thursday</b> 6pm-8pm PST</li>
							</ul>
							</td>
							</tr>
						</table>";
	...
	function editlessons(){
		...
		$fsql = "SELECT * FROM lessons where id = ".$vid;
		$fquery = $this->db->query($fsql);
		$fresult = $fquery->row_array();			
		
		$lessonname = $fresult['name'];
		$desc = $fresult['desc'];
		$id = $fresult['id'];
		echo json_encode(array('name'=>$lessonname,'desc'=>$desc,'id'=>$id));
	}
	function checktalknowlight(){
		$uid = $this->input->post('uid');
		$status = $this->user_model->checktalknowlight($uid);
		echo $status;exit;
	}
	
	function schoolsignup(){
		$this->layout->setLayout('layout_index');
		$this->layout->view('user/schoolsignup');
	}
	function promotion_links(){
		$this->load->library('pagination');
		$number_links = $this->input->post('number_links');
		$value_links = $this->input->post('value_links');
		$affiliate_id = $this->input->post('affiliate');
		

		$this->db->select('firstName');
		$this->db->from('profile');
		$this->db->where('uid', $affiliate_id);
		$affiliate_name = $this->db->get()->row()->firstName;
		
		$this->db->select('organizationName');
		$this->db->from('profile');
		$this->db->where('uid', $affiliate_id);
		$organization_name = $this->db->get()->row()->organizationName;
		
		$school_id = $this->user['uid'];
		$this->db->select('credit_balance');
		$this->db->from('user');
		$this->db->where('id', $school_id);
		$credit_balance = $this->db->get()->row()->credit_balance;
				
		$school_id = $this->user['uid'];
		$this->db->select('credit_balance');
		$this->db->from('user');
		$this->db->where('id', $school_id);
		$now = $this->db->get()->row()->credit_balance;					
		...
		$this->db->from('promotion_links');
		$count = $this->db->count_all_results();
		
		
		$config = array();
		$config['base_url'] = 'https://www.thetalklist.com/en/user/promotion_links';
		...
		$this->pagination->initialize($config);
		$this->session->set_flashdata('now', $now);
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		if(is_numeric($number_links)){
			if(is_numeric($value_links)){
				$total_amount = ($number_links * $value_links);
				for($i = 1; $i<=$number_links; $i++){
					...
					if($credit_balance >= $total_amount){
						$sql = "INSERT INTO promotion_links (school_id,code,affiliate_name,affiliate_id,organizationName,credit_value,redeemed,date) VALUES ( '{$school_id}','{$random}','{$affiliate_name}','{$affiliate_id}','{$organization_name}','{$value_links}','No','{$date}')";
						$query = $this->db->query($sql);
					}else{
						$message = "Your Credit Balance is".$now.".You do not have enough credits to issue these promotion links.";
						$this->session->set_flashdata('msg', $message);
						
						redirect('user/promotion_links');
					}		   
					//$random = $this->user['firstName'].rand();
		...
		if($query){		
				...
				$this->db->from('promotion_links');
				$this->db->where('school_id', $school_id); 
				$query_export = $this->db->get();
				$arr_export = $query_export->result();
				
				$sql_update = "UPDATE user set credit_balance = credit_balance - '{$total_amount}' WHERE id = '{$school_id}' ";
				$query_update = $this->db->query($sql_update);

			$this->layout->view('user/promotion_link',array('arr' => $arr,'links'=>$links,'arr_export'=>$arr_export));		
		}else{
			...
			$this->layout->view('user/promotion_link',array('arr' => $arr,'data'=>$info,'links'=>$links,'arr_export'=>$arr_export));	
		}
	...
	function members(){
		...
		$this->db->select('profile.*,promotion_links.*,countries.*');
		$this->db->from('promotion_links');
		$this->db->join('profile', 'promotion_links.school_id = profile.uid', 'inner');
		$this->db->join('countries', 'profile.country = countries.id', 'inner');	
		$this->db->where('promotion_links.school_id', $id);
		//$this->db->distinct('promotion_links.affiliate_id');
		
		$query_inner = $this->db->get();
		$info = $query_inner->result();
		
		$this->db->select('*');
		$this->db->from('profile');
		$this->db->join('user', ' profile.uid = user.id', 'inner');
		$this->db->join('countries', 'profile.country = countries.id', 'inner');
		//$this->db->join('promotion_links', 'user.refid = promotion_links.affiliate_id', 'inner');
		$this->db->where('profile.school_id', $id);	
		$this->db->where_in('user.roleId',array('1','2','3'));		
		$query_tutor = $this->db->get();
		$tutors = $query_tutor->result();
		 foreach($tutors as $tutor){
				//$querys = $this->db->query("select firstName as affiliate_name from profile where uid = '{$tutor->refid}'");
				//$result[] = $querys->row_array();
				$price[] = $this->db->select('firstName')
				  ->get_where('profile', array('uid' => $tutor->refid))
                  ->row()
                  ->firstName;
				  
			//$affi_name = $this->db->query("select firstName as affiliate_name from profile where uid = '"{$tutor->refid}"'");
		
			$data[] = (object) array_merge((array) $tutor, (array) $price);
		} 
		//print_r($data);
		
		//exit; 
		
		//$this->db->select('profile.*,promotion_links.*,user.*');
		$this->db->select('*');
		$this->db->from('profile');
		//$this->db->join('profile', 'promotion_links.school_id = profile.uid', 'inner');
		$this->db->join('user',' profile.uid = user.id', 'inner');
		$this->db->join('countries', 'profile.country = countries.id', 'inner'); 		
		$this->db->where(array('profile.school_id'=>$id,'user.roleId'=>0));
		
		$query_student = $this->db->get();
		$students = $query_student->result();
		
		$this->layout->view('user/members',array('info'=>$info,'tutors'=>$tutors,'studets'=>$students));
	}
	...
	public function updateProfile_pic(){
		...
		//$file = 'uploads/images/'.$filename.'.jpg';
	...
	//echo $data;
		...
		if($info){
			$sql = "UPDATE profile set pic = '{$imgname}' WHERE uid = '{$aj_uid}' ";
			$query = $this->db->query($sql);
			//echo "Succez";
			exit;
		}else{
	...
	public function links(){
		...
		$date = date('Y-m-d');
		for($i = 1; $i<=$number_links; $i++){
			$sql = "INSERT INTO promotion_links (school_id,code,affiliate_name,affiliate_id,credit_value,redeemed,date) VALUES ( '{$school_id}','123456','{$affiliate}','12341','{$value_links}','No','{$date}')";
			$query = $this->db->query($sql);
		}
		if($query){
			echo "Success";
		...
		$fetch = "Select * From promotion_links where school_id = '{$school_id}'";
		$this->layout->setData('promotion_links',$fetch);
		$this->layout->view('user/promotion_links');
	}
	public function autocomplete_affiliate(){
	
		$json = array($this->input->get("q"),$this->input->get("q"),"dasda");
		$this->load->database();

		 if($this->input->get("q") != ''){			
			$this->db->like('firstName', $this->input->get("q"));
			$query = $this->db->select('uid,firstName,organizationName as text')
								->from('profile')
								->join('user', 'profile.uid = user.id', 'inner')
								->where('user.roleId', 5)
								->limit(10)
								->get();
			$json = $query->result(); 
	...
	$term = $_REQUEST('query');
	$json = array($term,'dasa');
			 if($term != ''){			
			$this->db->like('firstName', $term);
			$query = $this->db->select('uid,firstName as text')
								->from('profile')
								->join('user', 'profile.uid = user.id', 'inner')
								->where('user.roleId', 5)
								->limit(10)
	...
	*/
	...
	public function delete_tutor_affi(){
		$id = $this->uri->segment(3);
		$this->db->where('id', $id);
		$this->db->delete('user');
		redirect('user/affiliateMembers');
	}
	...
	public function delete_link(){
		...
		$this->db->from('promotion_links');
		$this->db->where('id', $id);
		$credit_value = $this->db->get()->row()->credit_value;
		
		
		$this->db->select('redeemed');
		$this->db->from('promotion_links');
		$this->db->where('id', $id);
		$redeemed = $this->db->get()->row()->redeemed;
		
		$this->db->where('id', $id);
		$del = $this->db->delete('promotion_links');
		
		if($redeemed == 'No'){
			$sql_update = "UPDATE user set credit_balance = credit_balance + '{$credit_value}' WHERE id = '{$school_id}' ";
			$query_update = $this->db->query($sql_update);
			redirect('user/promotion_links');
		}else{
			redirect('user/promotion_links');
	...
	public function del_promo_link(){
	...
	public function tutor_scl(){
		 $this->load->library('email');
		 $school_id = $this->input->post('school');
		
		 $tutor_id = $this->user['uid'];
		
		 $this->db->select('firstName');
		 $this->db->from('profile');
		 $this->db->where('uid', $school_id);
		 $school_name = $this->db->get()->row()->firstName;
		 ...
		 $sql_update = "UPDATE profile set school_id = '{$school_id}' WHERE uid = '{$tutor_id}' ";
		 $query_update = $this->db->query($sql_update);
			
		// $sql = "INSERT INTO tutorSchool (school_id,tutor_id,school_name,status) VALUES ( '{$school_id}','{$tutor_id}','{$school_name}','0')";
		// $insert = $this->db->query($sql);
		 ...
		 if($query_update){
			...
			if($this->email->send()){ 
				 //$message = $this->session->set_flashdata("email_sent","Email sent successfully."); 
		...
		//$this->layout->view("user/profile/uid/".$tutor_id);

	}
	
	public function approve_tutor(){
		$id = $this->input->post('id');
		
		$sql_update = "UPDATE profile set school_id_approve = 1 WHERE uid = '{$id}' ";
		$query_update = $this->db->query($sql_update);
		
		if($query_update){
			echo $message = "Tutor is approved successfully.";
			exit;
		}else{
			echo $message = "Tutor Not Approved.";
			exit;
		}
		
	}
	
	public function decline_tutor(){
		$id = $this->input->post('id');	
		$sql_update = "UPDATE profile set school_id_approve = 0 WHERE uid = '{$id}' ";
		$query_update = $this->db->query($sql_update);
		
		if($query_update){
			echo "Tutor is Declined Successfully.";
			exit;
		}else{
			echo "Tutor Not Decline.";
			exit;
		}
		
	}
	
	public function affiliatePromotionLinks(){
		$this->load->library('pagination');
		$affiliate_id = $this->user['uid'];
		...
		$config = array();
		$config['base_url'] = 'https://www.thetalklist.com/en/user/affiliatePromotionLinks';
		$config['total_rows'] = $count;
		$config['per_page'] = 20;
		...
		$this->layout->view('user/affiliatePromotionLinks',array('arr_export'=>$arr_export,'links'=>$links));
	...
	public function affiliateMembers(){
		...
		$this->db->join('user',' profile.uid = user.id', 'inner');
		$this->db->join('countries', 'profile.country = countries.id', 'inner'); 		
		$this->db->where(array('user.refid'=>$affiliate_id,'user.roleId'=>0));	
		$query_student = $this->db->get();
		$students = $query_student->result();
		...
		$this->db->join('user',' profile.uid = user.id', 'inner');
		$this->db->join('countries', 'profile.country = countries.id', 'inner'); 		
		$this->db->where('user.refid',$affiliate_id);
		$this->db->where_in('user.roleId',array('1','2','3'));	
		
		$query_tutor = $this->db->get();
		$tutors = $query_tutor->result();
		
		$this->layout->view('user/affiliateMembers',array('tutors'=>$tutors,'students'=>$students,'affiliate_name'=>$affiliate_name));
		
	}
	
	public function finalrate(){
		$sql = "SELECT sum(onTime) AS sumontime, count(id) as countid, sum(clearReception) as sumclearreception, callerId from feedback GROUP BY callerId";
		$query = $this->db->query($sql);
		$result = array();
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
			...
			foreach ($result as $k=>$rate):
				echo $rate['callerId'].",";
				echo $onTimeAvg = $rate['sumontime']/$rate['countid'].",";
				echo $clearreceptionAvg = $rate['sumclearreception']/$rate['countid'].",";
				echo $finalavg = intval(($onTimeAvg+$clearreceptionAvg)/2 ).",";
				echo "<br>";
				
				echo $sql = "update profile set avgRate='{$finalavg}' where uid=".$rate['callerId'];
				//$query = $this->db->query($sql);
	...
	public function aiesecers(){
		$this->layout->setLayout('layout_index');
		$this->layout->view('user/aiesecers');
	}
	public function about_aiesecers(){
		$this->layout->setLayout('layout_index');
		$this->layout->view('user/about_aiesecers');
	}
	/* End */	
}?>		