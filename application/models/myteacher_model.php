...
class MyTeacher_model extends TL_Model{
	...
	public function getAll($uid,$page = 1,$perPage = 50) {
		...
		$sql = "SELECT myTeacher.*,profile.school_id,profile.firstName,profile.hRate,profile.school_id,profile.lastName,profile.school_id,profile.uid,profile.pic,user.readytotalk,user.fbshare from user, myTeacher  left join profile on profile.uid = myTeacher.tid where myTeacher.`sid` = {$uid} and profile.uid = user.id and `user`.`roleId`>=1 limit {$start},{$perPage}";
