...
function displayBuzzword(data, boxType) {
    ...
    for (var i = 0; i <  previousBw.length ; i++) {
        id = previousBw[i][0];
        word = previousBw[i][1];
        words += "<li style=\"" + MEDO_CSS_PROP.wordlistitem + "\"><a href=\"" + baseUrl + "/" + id + "\" style=\"" + MEDO_CSS_PROP.wordlink + "\">" + word + "</a></li>";
    }
    
    html += "<ul style=\"" + MEDO_CSS_PROP.wordlist + "\">" + words + "</ul>";
    html += "<a href=\"" + MEDO_HOST + "\" style=\"" + MEDO_CSS_PROP[MEDO_CURBOX_TYPE + '_logo'] + "\"></a>";
    html += "</div>";
    jQuery("#medo_buzzword").html(html);
    jQuery("#medo_buzzword .def").ThreeDots({ max_rows: 3 });
    jQuery("#medo_buzzword .def").append("<br/><a href=\"" + baseUrl + "/" + currentId + "\" style=\"" + MEDO_CSS_PROP.morelink + "\">more</a>");
}
