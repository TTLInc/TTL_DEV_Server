...
function chkpayemail(){
	if( $('#payemail').val() == '')
	{
		...
		document.getElementById('redpaymail').className += ' class_two';		
		document.getElementById('email_taken1_pay').style.display = 'none';
		document.getElementById('vremail_pay').style.display = 'none';
		document.getElementById('remail_pay').style.display = 'block';
		return false;
	}
	else
	{
		
		document.getElementById('remail_pay').style.display = 'none';
	}
	...
	document.getElementById('redpaymail').className += ' class_two';	
	document.getElementById('email_taken1_pay').style.display = 'none';	
	 document.getElementById('vremail_pay').style.display = 'block';
	return false;
	}
	else
	{
	...
	$.ajax({
				  type:     "GET",
				  url: '<?php echo Base_url("user/paypalcheck_affi");?>',
				  //data: { multiLang: multiLang},
				  data: {name:payemail},
				  dataType: "jsonp",
				   jsonp: 'callback',
				  ...
				  success: function(msg){
					  if(msg.success==false || msg.success=='false')
					{
						...
						document.getElementById('redpaymail').className += ' class_two';	
						//document.getElementById('vremail').style.display = 'none';
						document.getElementById('email_taken1_pay').style.display = 'block';
					}
					else
						
					{
					
							chekpayEmailTaken123(msg);
							//myfunction(msg);
...
function chekpayEmailTaken123(data){ 
  if(data.success == true || data.success=='true')
  { 
						//document.getElementById("ered").style.border="none";
						$('#redpaymail').removeClass('class_two');
...
if( $('#country').val() == '')
	{
		document.getElementById('redcountry').className += ' class_two';	
		document.getElementById('reqcountry').style.display = 'block';
		return false;
	}
	else
	{
		document.getElementById("redcountry").style.border="none";
		document.getElementById('reqcountry').style.display = 'none';
	}
	if( $('#city').val() == '')
	{
		document.getElementById('redcity').className += ' class_two';	
		document.getElementById('reqcity').style.display = 'block';
		return false;
	}
	else
	{
		document.getElementById("redcity").style.border="none";
...
<div class="index-banner" style="min-height:600px;">
	<div class="container" style="max-width:100%; position:relative">
		...
		<h1 class="banner_slogan_1">Make money by promoting international conversations!</h1>
		</div>
		<div class="school_main">
			<div class="school_left">
				<div class="school_content_signin">					
					Learn about our peer to peer educational model. You refer students to sign up, and we handle all the backend details.
				</div>
			...
			<div class="school_right">
				<div class="school_right_content" style="width:92%;">
					<div class="sc_information" style="width:100%;">
						<h3 style="margin-left:3%; margin-top:2%;">
							Signup to start earning 10% now!
						</h3>
					</div>
					...
					<div class="school_signupform">
						<form style="display:block;" action="<?php echo Base_url('user/registerDo');?>" method="POST" id="registerFormindx">
							...
							<div class="sf_input" id="redoname">
								<input style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" id="firstName" type="text" value="" name="organizationName" placeholder="<?php echo "Organization Name (optional)";?>" size="25" class="txtbox" />
								<span style="color:red;display:none;margin-left:5%;font-size:17px;padding-top:10px;" id=""><?php echo $enterfname;?></span> <span id="firstName_required" style="color:#DFC964;margin-left:5%;font-size:15px;display:none;"><b><?php echo $enterfname;?></b></span> </div>						
						  ...
						  <div class="sf_input" id="redpaymail">
						    <input style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" id="payemail" type="text" value="" name="payment_account" placeholder="<?php echo "PayPal account email to receive payments";?>" size="25"  class="txtbox"/> </span>
							<span id="email_required_pay" style="color:#DFC964;font-size:15px;font-size:15px;display:none;padding:1em;margin-left:5%; "><b><?php echo "Enter PayPal Email";?></b></span> <span id="email_invalid_pay" style="color:#DFC964;margin-left:5%;display:none;padding:1em"><b><?php echo "Enter Valid PayaPal email";?></b></span> <span style="color:red;display:none;font-size:17px;margin-top:10px; margin-left:5%;" id="remail_pay"><?php echo "Enter PayPal email";?></span> <span style="color:red;display:none;margin-left:5%;font-size:17px;margin-top:10px;" id="vremail_pay"><?php echo "Enter valid PayPal email";?></span> <span id="email_taken1_pay" style="color:red;display:none;font-size:17px;margin-top:10px;margin-left:5%;"><b><?php echo "PayPal email is Already taken";?></b></span> </div>
					
						  <div class="sf_input" id="redcountry">
						  <input style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" type="text" class="raduisSelect" placeholder="<?php echo "Country"; ?>" id="country" name="country"/>
						<span style="color:red;display:none;margin-left:5%;font-size:17px;padding-top:10px;" id="reqcountry"><?php echo "Enter Country";?></span> 
						</div>
						  
						  <div class="sf_input" id="redcity">
						  <input style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" type="text" class="raduisSelect" placeholder="<?php echo "City"; ?>" id="city" name="city" />
						<span style="color:red;display:none;margin-left:5%;font-size:17px;padding-top:10px;" id="reqcity"><?php echo "Enter City";?></span> 
						</div>
...
--> 
						 <div class="sf_input sf_input_pass" id="rpass">
						<input type="password" style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" name="password" id="password" placeholder="Password" size="25">
						<span style="color:red;display:none;font-size:17px;margin-top:20px;margin-left:5%;" id="pass"><?php echo "Enter Password.";?></span>
						<span style="color:red;display:none;font-size:17px;margin-top:20px;margin-left:5%;" id="passlong1"><?php echo "Min Six Character";?></span>
						</div>
						<div class="sf_input sf_input_pass" id="rcpass">
						<!-- <input type="password" style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" id="confirmPassword" name="confirm_password"  placeholder="Confirm" size="25">  -->
						<input style="width:90%; margin-left:5%; margin-top:3%; background-color:inherit; height:35px; border:2px solid #ccc; padding-left:20px" autocomplete="off" type="password" value=""  id="confirm_password12" name="cpassword" placeholder="<?php echo $lCPASSWORD;?>" size="25"/>
				
							<span style="color:red;display:none;font-size:17px;margin-top:20px;margin-left:5%;" id="cpass12"><?php echo $confpass;?></span> <span style="color:red;display:none;font-size:17px;margin-top:33px;margin-left:5%;" id="cpass12d"><?php echo $passmissmatch;?></span> 
						</div > 
					
					
						<input style="margin:3% 5%; background-color:#3399CC; border:none; color:#fff; border-radius:5px; width: 140px; height: 40px;" name="signup" type="button" onclick="return validateFrm();" value="<?php echo $lSIGN_UP;?>" class="signup_btn" id="registerButton" >
