...
class Multi extends TL_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model(array('lookup_model','multi_model'));
		if(!isset($_SESSION)) {
			 session_start();
		}
  ...
  public function UpdateGroup()
  {
	...
	{
		$cid= $_POST['classId'];
		$sql="select isChatSent,tutorPresent from gropsession where gropsessionId='{$cid}'";
		$query=$this->db->query($sql);
		if($query->num_rows() > 0){
					$result = $query->row_array();
	...
	public function removestu()
	{
		$gid=$_POST['groupid'];
		$this->multi_model->RemoveParitcipant($gid);
		die;
	}
