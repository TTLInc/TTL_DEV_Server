<?php session_start();?>
<?php $this->layout->setLayoutData('content_for_layout_title','Credit Status');?>
           ...
           <!-- <td><input type="checkbox" name="ids[]" value="<?php echo $val['id'];?>" /></td>-->
            ...
            ?>
             ...
             <td>
				<?php 
					 ...
					 echo "$". number_format(@$val['tutorhRate'],2,'.','');
			...
			<td>
				<?php
					...
					$hourdiff = 72;
					echo date('Y-m-d H:i:s',$currentDtStr + ($hourdiff * 3600) );
				?>
			</td>
			<td> <?php 
		if($val['PayDate'] != '0000-00-00')
		{
			...
			echo date("Y-m-d", strtotime($val['PayDate']));
			 ...
			 <td> <?php echo @$val['tconnect']; ?></td>
			  <td> <?php echo @$val['sconnect']; ?></td>
			   <td> <?php echo "$". number_format(@$val['money'],2,'.',''); ?></td>
            ...
            <?php 
				...
				<?php	
					...
					<td id="actions" class="<?php echo $class; ?>" >
						<?php if($val['postpone'] == 1 || $val['approve'] == 1 || @$val['is_Deleted'] ==1 || @$val['status'] == 'paid'){?>
						<img alt="Postpone" src="<?php echo base_url('/images/postpon-gray.png');?>" />
						<?php }else{ ?>
						...
						<?php } ?>
						<?php if($val['approve'] == 1 || @$val['is_Deleted'] ==1 || @$val['status'] == 'paid'){?>
					 <img alt="Approve" src="<?php echo base_url('/images/approve-gray.png');?>" /> 
						<?php }else{?>
						...
						<?php } ?>
						<?php if(@$val['is_Deleted'] == 1 || $val['approve'] == 1 || @$val['status'] == 'paid') { ?>
						<img alt="delete" src="<?php echo  base_url('images/close-gray.png')?>" />
						<?php }else {?>
...
setTimeout('showmenu("financial",4)',1000);
function del(_id){
	...
	if(window.confirm('Are you sure you want to approve this payment?')){
		$.post('<?php echo Base_url("admin/disputes_approve")?>',{id:_id,approveval:_val},function(msg){
			...
			if(json.status){
				...
				alert('Credit status has been successfully approved.');
				window.location.reload(true);
			}
			else{
				alert(json.msg);
