...
class Banner_model extends TL_Model{
	...
	public function loadBannerHome(){
	...
	public function home_banner_add($data){
		return $this->db->query($this->db->insert_string('home_banners',$data));
	}
	
	public function home_banner_del($id){
		foreach($id as $v){
			$sql = "DELETE FROM home_banners WHERE  id={$v}";
			$this->db->query($sql);
		}
		return $id;
	}
	
	public function home_banner_edit($data,$id){
		return $this->db->query($this->db->update_string('home_banners',$data,"id={$id}"));
	}
	
	public function home_get_banner($id){
		$sql = "SELECT * FROM home_banners WHERE home_banners.id = {$id}";
		$query = $this->db->query($sql);
		if ( $query->num_rows() > 0) {
			$result = $query->row_array();
		}
