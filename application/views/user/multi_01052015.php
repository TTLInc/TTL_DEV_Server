...
<script src="//static.opentok.com/webrtc/v2.2/js/opentok.js" type="text/javascript" charset="utf-8"></script>
...
$(window).load(function() {
		 ...
		 $.post('<?php echo base_url('multi/checkisSent');?>',{classId:cid},function(msg){
			if (String == msg.constructor)
					...
					{
						//$('.send_chat_ipt').val('Welcome! Use earbuds for best audio. Wait for tutor video to appear. Have fun!');
						//addMsg('Welcome! Use earbuds for best audio. Wait for tutor video to appear. Have fun!');	
...
<script type="text/javascript">
	...
	$(window).load(function () {
		if(_startTime > 0){
			addMsg("Session will start as soon. When both parties arrive, feel free to chat while you wait for the timer countdown.");
		}
 ...
 $(document).ready(function(){
	...
	function checkStatus(){
		...
		if(_endTime>0){
			...
			if(user.type=='t'){ document.location.href='<?php echo Base_url('user/calendar');?>';}
			feedBack();
...
addStream(event.streams[i]);
	 ...
	 var publishProps = {height:$('#publisherContainer').height(), width:$('#publisherContainer').width()};
		...
		if(user.type=='t')
		{	
			publisher = OT.initPublisher(apiKey, 'publisherContainer', publishProps);
		} 		
  ...
  function subscribeToStreams(streams) {
	...
	function turnOnMyAudio(){
		...
		if (OT.checkSystemRequirements() != OT.HAS_REQUIREMENTS) {
			...
			publisher = OT.initPublisher(apiKey, parentDiv.id, publisherProperties);
...
$('#Room_sear_result_dict').html(_html);
            ...
            <!--for dict script end-->
                    ...
                    <div class="TabbedPanelsContent blu-bgexpnd" id="tab-7">
						...
						?>
                        <li><?php echo $j=$i+1;?>.<a target='_blank' href="<?php echo $pdfurl; ?>"><?php echo $testScenario[$i]['Title']?></a></li>
