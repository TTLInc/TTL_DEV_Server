<?php $this->layout->setLayoutData('content_for_layout_title','LMS Document list');?>
<?php $this->layout->setLayoutData('content_for_layout_links','<a href="'.base_url('admin/adddocument').'">Add new Document</a>');?>
...
<table class="data_table">
    ...
    <tbody>
    <?php $j = 0; foreach ($documents as $key => $val) :?>
        <tr id="id_<?php echo $val['id'];?>">
            <td><input type="checkbox" name="ids[]" value="<?php echo $val['id'];?>" /></td>
            <td><?php echo $val['id'];?></td>
            <td><?php echo $val['title'];?></td>
             <td>
				<a target="_blank" href="<?php echo base_url().'uploads/LMS/'.$val['document_file']; ?>">
				<?php echo $val['document_file'];?>
				</a>
			</td>
             <td><?php echo $val['status'];?></td>
            <td>
            <a href="<?php echo base_url('admin/editdocument/id/'.$val['id']);?>"><img alt="Edit" src="<?php echo base_url('/images/cm_pencil.png');?>" /></a>
...
function del(_id){
	if(window.confirm('Are you sure you want to delete this?')){
		$.post('<?php echo Base_url("admin/document_del")?>',{id:_id},function(msg){
