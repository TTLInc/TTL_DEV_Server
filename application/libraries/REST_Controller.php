    ...
    */
    ...
    public function __construct($config = 'rest')
    {
        ...
        if (!$this->input->is_ajax_request() && config_item('rest_ajax_only')) {
            $response = array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'Only AJAX requests are accepted.');
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('force_https') && !$this->_detect_ssl()) {
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'Unsupported protocol'), 403);
        }
        ...
        if (config_item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'Invalid API Key '.$this->rest->key), 403);
        }
        ...
        if (config_item('rest_enable_keys') && $use_key && !empty($this->rest->key) && !$this->_check_access()) {
            ...
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'This API key does not have access to the requested controller.'), 401);
        }

        // Sure it exists, but can they do anything with it?
        if ( ! method_exists($this, $controller_method)) {
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'Unknown method.'), 404);
        }
        ...
        if (config_item('rest_enable_keys') && !empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') && !$this->_check_limit($controller_method)) {
                $response = array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'This API key has reached the hourly limit for this method.');
            ...
            $response = array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'This API key does not have enough permissions.');
    ...
    protected function _check_php_session()
    {
        ...
        if (!$this->session->userdata($key)) {
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'Not Authorized'), 401);
        }
    }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if ($digest['response'] != $valid_response) {
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'Invalid credentials'), 401);
            exit;
        }
    }
    ...
    protected function _check_whitelist_auth()
    {
        ...
        if ( ! in_array($this->input->ip_address(), $whitelist)) {
            $this->response(array(config_item('rest_status_field_name') => 1, config_item('rest_message_field_name') => 'IP not authorized'), 401);
        }
    }
