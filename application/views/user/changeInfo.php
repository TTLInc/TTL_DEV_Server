...
$arrVal = $this->lookup_model->getValue('1392', $multi_lang);
$sharetofacebook = $arrVal[$multi_lang];
?>
...
.pro-settg .tab-cnt .sub-cnt .cnt-rw label{
	width:110px;
}
</style>
...
<form name="cancelInfoFrm" id="cancelInfoFrm" action="<?php echo Base_url('user/changeInfo');?>" method="POST">
								...
								<!--	   <div class="cnt-rw">
                                        	<label><?php //echo "Select School for display tutor";?>:</label>
											
													<input style="width:25px" type="checkbox" name="school" value="1" <?php //if(@$profile['sc_flag']=='1'){echo 'checked';} ?>>
								   ...
								   <?php endif;?>
									...
									<form action="<?php echo Base_url('user/changeInfo');?>" method="POST"  onsubmit="return validateFormCon5();">
                                        ...
                                        <div class="cnt-rw">
                                            ...
                                            <input autocomplete="off"   name="hiddenRole" id="hiddenRole" type="checkbox" class="chek-bx" value="0" <?php if(@$profile['hiddenRole']=='1'){echo 'checked';} ?>>
                                        </div>
										<?php endif;?>
										<div class="cnt-rw">
                                        	<label><?php echo $sharetofacebook;?>:</label>
                                            <input autocomplete="off"   name="fbshare" id="fbshare" type="checkbox" class="chek-bx" value="1" <?php if(@$profile['fbshare']=='1'){echo 'checked';} ?>>
                                        </div>
                                        <div class="cnt-rw no-brd">
                                        	<label>&nbsp;</label>
                                            <input type="submit" value="<?php echo $lsave;?>" class="st-btn"> <input type="button" value="<?php echo $lCANCEL;?>" class="st-btn" onclick="cancelInfo();">
...
function hide(){
	...
	$('#country').change(function(){
		...
		$.getJSON('<?php echo Base_url("user/getProvices");?>',{cid:_cid},function(provices){
			...
			$("select#province").prepend("<option value='' selected='selected'></option>");
