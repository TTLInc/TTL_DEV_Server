...
class Search_model extends TL_Model{
	...
	public function getResult($arCnd=array(), $offset=0, $limit=20)
	{
		...
		if (!empty($arCnd)) {
			...
			if (isset($arCnd['today']) and (!empty($arCnd['today']))) {
				...
				$qryTimeSlot = " INNER JOIN `timeslot` ON  `profile`.`uid` = `timeslot`.`uid` ";
			}
		...
		$cnd .= " and (`profile`.`pic` != '') and (`profile`.`personal` != '' OR `profile`.`professional` != '' OR `profile`.`academic` != '' ) and `quarantine` != '1' and `hiddenRole` = '0'";
		...
		$sqlTotal = "SELECT COUNT(`user`.`id`) as `total` from `user` INNER JOIN `profile` ON `user`.`id` = `profile`.`uid` WHERE `user`.`roleId` IN (1,2,3) $cnd ";
	...
	public function getFetureTeachers(){
		...
		$sql = "select profile.uid,profile.pic,profile.firstName,profile.lastName,profile.Lat,profile.Lng,countries.country , provices.provice ,profile.city,user.roleId,profile.hRate,profile.school_id FROM profile INNER JOIN user ON user.id = profile.uid INNER JOIN countries ON profile.country = countries.id INNER JOIN provices ON profile.province = provices.id INNER JOIN newtutors ON newtutors.uid = profile.uid  WHERE (newtutors.uid = profile.uid OR user.roleId = 3 )   AND user.`hiddenRole` = 0  AND user.`quarantine` =0 order by RAND()";
		$query = $this->db->query($sql);
		$results = array();
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
	...
	public function getFreeFetureTeachers(){
		...
		$sql = "select profile.uid,profile.pic,profile.firstName,profile.lastName,profile.Lat,profile.Lng,countries.country , provices.provice ,profile.city,profile.school_id,user.roleId,profile.hRate FROM profile INNER JOIN user ON user.id = profile.uid INNER JOIN countries ON profile.country = countries.id INNER JOIN provices ON profile.province = provices.id INNER JOIN newtutors ON newtutors.uid = profile.uid WHERE (newtutors.uid = profile.uid OR user.roleId = 3 ) AND profile.free_session = 'y' AND user.roleId = 2 AND user.`hiddenRole` = 0 order by RAND()";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
			foreach($result as $k=>$v){
	...
	public function getVideoResult1($searchData){
		...
		$select = "SELECT p.firstName,p.lastName,u.quarantine,p.pic,l.source,l.desc,l.price,l.uid FROM  lessons as l INNER join profile as p on l.uid=p.uid INNER join user as u on u.id=p.uid WHERE 1=1 AND ";
		...
		if($searchData['sort'] != ''){
			$wherefields .= " ORDER BY l.views DESC ";
		}
