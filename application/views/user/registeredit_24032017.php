...
$arrVal = $this->lookup_model->getValue('1307', $multi_lang);	$lngAddDetailLater = $arrVal[$multi_lang];
$arrVal = $this->lookup_model->getValue('1311', $multi_lang);	$lngGenderEr = $arrVal[$multi_lang];
$arrVal = $this->lookup_model->getValue('1312', $multi_lang);	$lngCellEr = $arrVal[$multi_lang];
...
function checkAll(){
	...
	if( typeof( $('input[name^=gender]:checked').get(0) ) == 'undefined' && role < 4 ){
		checkTerms = false;
		showInfo('gender','<?php echo $lngGenderEr;?>');
		return;
	}
	else {
		checkTerms = true;
	...
	<?php
	if($this->session->userdata('roleId')=="1" and (isset($upgradeaccount) and $upgradeaccount==1)) {
	?>
	if( $('#cell').val() == ''	){
		showInfo('cell','<?php echo $lngCellEr;?>');
		return;
	}
	else{
		hideInfo('cell');
...
function submitData() {
 ...
 if(role==4 || role==5)
 {
	...
	$.post('<?php echo Base_url("user/registereditDo");?>',_data,function(msg){
		...
		if(json.success){
			<?php 
			if (strpos($_SERVER['HTTP_REFERER'], 'user/profile') !== false) {
				$variable = $_SERVER['HTTP_REFERER'];
				$httprefer = substr($variable, 0, strpos($variable, "?"));
			?>
				document.location.href = '<?php echo $httprefer;?>';
			<?php } else { ?>
...
$(function(){
	...
	$('#country').change(function(){
		...
		/* added by haren for new requirment */
		$('select#areacode').empty();
		$("#disareacode").val("");
		var coid = $(this).val(); 
		var cupdate ='cid='+coid;
					$.ajax({
								type:'POST',
								data:cupdate,
								url:'<?php echo base_url('user/getcountrycode/');?>',
								success:function(msg){
										...
										var newcode= "+" + json.Country_Code;
											$('#areacode').append(
														$("<option></option>")
														  .attr("value", newcode)
														  .text(newcode)
													);
											$('#disareacode').val(newcode);
									}
							});
	...
	$('#doregister').click(function(){checkAll();});
...
<div class="regist registeredit">
	...
	<div class="regist_mid_2 rig-struct">
		<dl id="showInfo" style="color:red;font-size:16px;"></dl>
		<dl class="content">
			<dd>
				<select class="raduisSelect" id="roleId" hidden>
                    ...
                    <option value="1" <?php if($this->session->userdata('roleId') == 1 || $this->session->userdata('roleId') == 2) {?>selected<?php }?>><?php echo $member;?></option>
				...
				<?php if($this->session->userdata('roleId') == 0){$rl=$member;} 
					   if($this->session->userdata('roleId') == 1){$rl=$member;}
				...
				<div><h2 style="width:1000px !important"><?php echo $rl."  ".$Registration; ?></h2></div>
			</dd>
		</dl>
		<?php if($this->session->userdata('roleId') < 4) {?>
		<dl style="clear:both;" class="content">
			<dt>
				<?php $arrVal = $this->lookup_model->getValue('114', $multi_lang);echo $arrVal[$multi_lang];?>:
			</dt>
			<dd>
				<span class="male">
					<input type="radio" name="gender" id="gender1" value="0" <?php if($profile['gender'] == 0 and !empty($profile['gender'])){echo 'checked';} ?> />&nbsp;
					<label for="gender1">
						<?php $arrVal = $this->lookup_model->getValue('115', $multi_lang);echo $arrVal[$multi_lang];?>
					</label>	
				</span>
			</dd>
			<dd>
				<span class="male">
					<input type="radio" name="gender" id="gender2" value="1" <?php if($profile['gender'] == 1){echo 'checked';} ?>/>&nbsp;
					<label for="gender2">
					<?php $arrVal = $this->lookup_model->getValue('116', $multi_lang); echo $arrVal[$multi_lang];?>
					</label>
				</span>
			</dd>
		</dl>
		...
		<!--<dl <?php echo $member;?> id="membershipInfo">
          <dt><?php echo $MEMBERSHIP_TYPE;?>:</dt>
          <dd>
				<span class="male"><?php	$arrVal = $this->lookup_model->getValue('1235', $multi_lang);?>
                <input type="radio" id="universal_roleId2" name="universal_roleId" value="0" class="radio" <?php if($this->session->userdata('roleId') == 0){ echo "checked";}?> /> <label for="universal_roleId2"><?php echo $arrVal[$multi_lang];?></label></span>
                <br/>
                <span class="male"><?php	$arrVal = $this->lookup_model->getValue('1234', $multi_lang);?>
                <input type="radio" id="universal_roleId1" name="universal_roleId" value="1" class="radio" <?php if($this->session->userdata('roleId') == 1){ echo "checked";}?> /> <label for="universal_roleId1"><?php echo $arrVal[$multi_lang];?></label></span>
          </dd>
        </dl>   -->
			<input type="radio" id="universal_roleId" name="universal_roleId" value="<?php echo $this->session->userdata('roleId');?>" class="radio" checked="checked"  hidden /> 	
		<?php }else {?>
			<input type="radio" id="universal_roleId" name="universal_roleId" value="<?php echo $this->session->userdata('roleId');?>" class="radio" checked="checked"  hidden /> 
		<?php }?>
        <!-- End -->
		<dl style="clear:both;" class="content">
			...
			<dd>
				<input type="text" class="raduisSelect" placeholder="<?php echo $lFIRST_NAME;?>:" id="firstName" value="<?php echo @$profile['firstName'];?>"/><span class="icnred">*</span>
			</dd>
			<?php if($this->session->userdata('roleId') < 4 ) {?>
			<dd>
				<input type="text" class="raduisSelect" placeholder="<?php echo $lLAST_NAME;?>:" id="lastName" value="<?php echo @$profile['lastName'];?>"/><span class="icnred">*</span>
			</dd>
			<dd>
				<input type="text" class="raduisSelect w100" placeholder="<?php echo $lAGE;?>:" id="age" value="<?php echo @$profile['age'];?>"/><span class="icnred">*</span>
			</dd>
			<?php }?>
		</dl>
			<?php if($this->session->userdata('roleId') ==4 || $this->session->userdata('roleId') ==5 ) {?>
			<dl style="clear:both;" class="clearfix" id="tutRateF">
				<dt class="pemail"><?php echo $pay_email ?>:<img src="<?php echo Base_url('images/arrow.png') ?>" /></dt>
				<dd>
					<input type="text" class="raduisSelect" placeholder="<?php echo $pay_email;?>" id="payment_account" value="<?php echo @$profile['payment_account'];?>" onblur="return chkpayemail();" /><span class="icnred">*</span>
				</dd>
			</dl>
			<hr />
		<?php }?>	
		...
		<dl style="clear:both;" class="content">
			...
			<dd>
				<input type="text" class="raduisSelect" placeholder="<?php echo $lFIRST_NAME. "/". $lLAST_NAME;?>" id="principle_name" value="<?php echo @$profile['principle_name'];?>" maxlength='20'/>&nbsp;<span class="icnred">*</span>
			</dd>
		</dl>
		<dl>
			<dt><?php echo $lEMAIL;?>:</dt>
			<dd>
				<input type="text" class="raduisSelect" disabled placeholder="E-mail:" id="email" value="<?php echo @$userInfo['email'];?>" maxlength='30'/><span class="icnred">*</span>
			</dd>
		</dl>
		<dl style="clear:both;">
			<dt><?php echo $address;?>:</dt>
			<dd>
				<input type="text" class="raduisSelect" placeholder="<?php echo $address;?>" id="address" value="<?php echo @$profile['address'];?>"/><span class="icnred">*</span>
			</dd>
		</dl><br><br>
	   <?php }?>
		<?php
		...
		<dl class="mt0 content">
				...
				<?php foreach ($countries as $key => $val){?>
					...
					<?php foreach ($val as $k => $v){?>
						<option value="<?php echo $k;?>" 
								<?php if($profile['country']==$k):?>selected<?php endif; ?>><?php echo $v;?></option>
					<?php }?>
					</optgroup>
				<?php }?>
				</select>
			...
			<dd>
				<input type="text" class="raduisSelect" placeholder="<?php echo $lcity; ?>" id="city" value="<?php echo $profile['city']; ?>" /> <span class="icnred">*</span>
			</dd>
		</dl>
		<?php if($this->session->userdata('roleId') ==4 || $this->session->userdata('roleId') ==5 ) {?>
			<dl style="clear:both;">
				...
				<dd>
					<input type="text" class="raduisSelect" placeholder="<?php echo $zipcode; ?>" id="zipcode" value="<?php echo @$profile[''];?>" onblur="return chkzip();" maxlength='8'/>&nbsp;<span class="icnred">*</span>
				</dd>
			</dl>
			<?php if($this->session->userdata('roleId') ==4) {?>
			<dl style="clear:both; height:120px;" class="clearfix" id="tutRateF">
				...
				<dd>
					<input type="text" class="raduisSelect" placeholder="$00.00" id="tutor_markup" onblur="return chkmarkup();"  value="<?php echo @$profile[''];?>" maxlength='6'/> &nbsp;<span class="icnred">*</span>
				</dd>
			</dl>
		<?php }
		}?>
	...
	<div class="regist_mid_2 regist_mid_3">
		<dl class="tooltip-nw content" id="tooltip-nw-id">
			<dt class="regci">
				<span style="float:left;font-weight: bold;color: #0D5782;font-size: 14px;">
					<?php $arrVal = $this->lookup_model->getValue('122', $multi_lang); echo $arrVal[$multi_lang];?>:
				</span>
				<img src="<?php echo Base_url('images/arrow.png') ?>" style="float:left;" />
			</dt>
			<?php 
			$cel1=explode('-',$profile['cell']);
			?>
			<dd class="cell_phone_dl" style="display:none;">
            	<input  type="text" disabled="disabled" id="disareacode" class="raduisSelect" style="width:auto; max-width:50px;" value="<?php echo @$cel1[0];?>"  />
				<select name="areacode" id="areacode" class="raduisSelect" style="display:none;">
				</select>
			</dd>
			<dd class="cell_phone_dl" style="display:none;">
				<!--<input type="text" maxlength="30" class="raduisSelect" placeholder="XXX-XXXX-XXXXXX"id="cell" value="<?php echo $profile['cell']; ?>" /><em class="ajayhifen">-</em>-->
				<i style="display:inline-block; vertical-align:middle;"><img src="<?php echo base_url()."images/phone.png";?>" /></i>
                <input type="text" maxlength="30" class="raduisSelect" placeholder="Enter Phone Number" name="cell" id="cell" value="<?php echo @$cel1[1];?>" />
                <!--<em class="ajayhifen">-</em>-->
                <br/>
                <span style='margin-left:35px;line-height:1.8em;font-size:12px'>(For SMS texts)</span>             
			</dd>
			<!--<dd>
		...
		<!--
		<dl>
			<dt><?php $arrVal = $this->lookup_model->getValue('129', $multi_lang); //echo $arrVal[$multi_lang];?>:</dt>
			<dd>
				<input type="text" class="raduisSelect" disabled placeholder="E-mail:" id="email" value="<?php echo @$userInfo['email'];?>"/> *
			</dd>
			<dd>
				<input type="text" class="raduisSelect" placeholder="Confirm E-mail:" id="confirmEmail" value="<?php echo @$get['confirmEmail'];?>"/>
			</dd>
		</dl>
		-->
		<?php 
		if($this->session->userdata('roleId') == 1 and @$upgradeaccount==1){	
			$tutStyle = "";
			$lngStyle = "";
		}else{
		...
		?>
		<dl id="tutRateF" <?php echo $tutStyle; ?> class="content">
			...
			<dd><span style="font-size:16px;">$</span><input type="text" class="raduisSelect" placeholder="__.__" id="hRate" value="<?php echo $profile['hRate'] ?>"/></dd>
		</dl>
		<hr style="clear:both;" />
		<?php if($this->session->userdata('roleId') > 0 and $this->session->userdata('roleId') < 4) {?>
		<dl class="content">
			<dt><label><?php echo $l1ST;?> <?php echo $first_language;?>:</label></dt>
			<dd><?php echo form_dropdown('nativeLanguage',$langs,$profile["nativeLanguage"],' id="nativeLanguage" class="raduisSelect"');?></dd>
		</dl>
		<dl class="content">
			<dt><label><?php echo $l2ND;?> <?php echo $first_language;?>:</label></dt>
			<dd><?php echo form_dropdown('otherLanguage',$langs,$profile["otherLanguage"],' id="otherLanguage" class="raduisSelect"');?></dd>
		</dl>		
		<hr />
		<?php }?>
	</div>
	<?php }?>
	<?php
	$arrVal = $this->lookup_model->getValue('131', $multi_lang);$ihaveread = $arrVal[$multi_lang];
	$arrVal = $this->lookup_model->getValue('378', $multi_lang);$terms = $arrVal[$multi_lang];
	$arrVal = $this->lookup_model->getValue('377', $multi_lang);$privacy = $arrVal[$multi_lang];
	$arrVal = $this->lookup_model->getValue('243', $multi_lang);$andagree = @$arrVal[$multi_lang];
	$arrVal = $this->lookup_model->getValue('244', $multi_lang);$andagreeit = @$arrVal[$multi_lang];
	$arrVal = $this->lookup_model->getValue('371', $multi_lang);$vregister = @$arrVal[$multi_lang];
	$arrVal = $this->lookup_model->getValue('429', $multi_lang);$bsubmit = @$arrVal[$multi_lang];
	?>
	<div class="regist_mid_2 regist_mid_4" style="clear:both;">
		<div class="checkbox"><input type="checkbox" name="checkbox_terms" id="checkbox_terms" />&nbsp;
			<a href="<?php echo Base_Url('article/terms');?>"><u><?php echo $terms;?></u></a><?php /*&nbsp;<?php echo $andagree;?>.*/?>
		</div>
		<div class="checkbox"><input type="checkbox" name="checkbox_privacy" id="checkbox_privacy"  />&nbsp;
			<a href="<?php echo Base_Url('article/privacy');?>"><u><?php echo $privacy;?></u></a><?php /*&nbsp;<?php echo $andagreeit;?>*/?>
		</div>
	</div>
	<div class="regist_mid_2 registereditbtn" align="right">
		<input type="hidden" value="<?php echo $upgradeaccount;?>" id="upgradeaccount" name="upgradeaccount" />		
		<input type="hidden" value="<?php echo @$_SERVER['HTTP_REFERER']; ?>" name="httpref" id="httpref" />
		<?php 
		if (isset($upgradeaccount) and $upgradeaccount==1) {
			$arrVal = $this->lookup_model->getValue('320', $multi_lang);
			?>
			<input type="button" class="btnBlue" value="<?php echo $bsubmit;?>" id="doregister" />
		<?php
		} else {?>
		<input type="button" class="btnBlue" value="<?php echo $bsubmit;?>" id="doregister" />		
		<!--<input type="button" class="btnBlue" value="<?php echo $lngAddDetailLater;?>" id="add_detail_later" onclick="javascript:self.location.href='<?php echo Base_Url("search/search");?>'" />-->
		<?php }?>
	</div>
</div>
<style>
...
$(document).ready(function(){
	$("input[name*=universal_roleId]").change(function(){
		$("#tutRateF").hide();
		if($(this).val()==1){
			$("#tutRateF").show();
		}
...
<?php
if($this->session->userdata('upAct')==1) {
	$this->session->unset_userdata('upAct');
	?>
<script type="text/javascript">
$(document).ready(function(){
	$('#alertMsg').dialog({
...
<?php $arrVal = $this->lookup_model->getValue('1314', $multi_lang); $lngUpActInfo = $arrVal[$multi_lang];?>
<div id="alertMsg" style="display:none;">
	<div style="width:260px;line-height:1.6em;" class="txtblue">
	<?php echo $lngUpActInfo;?>
	</div>
</div>
<?php }?>