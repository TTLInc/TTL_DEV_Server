...
class Pay_model extends TL_Model{
	...
	*/
	...
	public function getNotPayed() {
		$sql 	= "SELECT * from disputes WHERE is_completed=1 AND paystatus = 0 AND postpone = 0 AND approve = 0 AND cid IN (select id from class) AND is_Deleted != 1 AND cid > 10660 order by createAt desc ";
		$query 	= $this->db->query($sql);
	...
	public function update_tutor_disputes($tid,$sid,$createAt) {
		$sql 	= "SELECT id,fee,t_hrate from disputes WHERE tid={$tid} AND sid = {$sid} AND createAt = '{$createAt}'";		
		...
		if ($query->num_rows() > 0) {
			...
			$approvesql = "update disputes set approve=1 where tid={$tid} AND sid = {$sid} AND createAt = '{$createAt}'";			
			$query = $this->db->query($approvesql);
		}
	}
...
/*
	...
	*/
	...
	public function GetSchoolPaymentForSearch($start,$search = '',$limit,$sortorder,$sort) {
		$sql ="SELECT * FROM `school_payment_transaction` WHERE (school_id like '%{$search}%' or school_name like '%{$search}%')";
		$query 	= $this->db->query($sql);
		$result = array();
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
	...
	public function GetUncompleteSession($start,$search = '',$limit,$sortorder,$sort,$dsearch) {
		...
		if($sort=='tname')
		{
			$sort="p.firstName";
				...
				if($search=='')	
				{	
					$sql = "SELECT  c.*, p.firstName,p.email from class as c,profile as p  where (c.sid = p.uid or c.tid = p.uid) and MONTH(c.createAt)='{$mon}' and YEAR(c.createAt)='{$year}' and (s_attend=0 OR session_type = 'free') group by c.id  order by {$sort} {$sortorder} LIMIT {$start}, {$limit}";
				}
				else
				{
	 ...
	 $result = 0;
		if($dsearch=='')
				...
				{	
					...
					$sql = "select count(DISTINCT c.`id` )as num from class as c,profile as p  where (c.sid = p.uid or c.tid = p.uid) and MONTH(c.createAt)='{$mon}' and YEAR(c.createAt)='{$year}' and session_type = 'free'";
				}
				else
				{
					...
					$sql = "SELECT count(DISTINCT c.`id`) as num from class as c,profile as p  where (c.sid = p.uid or c.tid = p.uid) and MONTH(c.createAt)='{$mon}' and YEAR(c.createAt)='{$year}' and  session_type = 'free' and (c.id like '{$search}' OR p.uid like '{$search}' OR p.email like '%{$search}%' OR p.firstName like '%{$search}%' OR p.lastName like '%{$search}%')";
				}
	...
	public function updatepaywallMoney($payinfo){
		$uid 	= $payinfo['uid'];
		$money = $payinfo['money'];
		$coupon_credits = $payinfo['coupon_credits'];
		$purchased_credits = $payinfo['purchased_credits'];
		$sql 	= "SELECT money,purchased_credits,coupon_credits from profile WHERE uid = '{$uid}'";
		$query 	= $this->db->query($sql);
		$result = false;
		if ($query->num_rows() > 0) {
			$result 	= $query->row_array();
			@$result1 	= @$result['money'] + $money;
			@$result2 	= @$result['purchased_credits'] + $purchased_credits;
			@$result3 	= @$result['coupon_credits'] + $coupon_credits;
			$sql 		= "UPDATE profile SET money = '{$result1}',purchased_credits = '{$result2}',coupon_credits = '{$result3}' WHERE uid = '{$uid}'";
			$query 		= $this->db->query($sql);

			//-- affiliate earning start-------
			$refsql 	= "SELECT refid from user WHERE id = '{$uid}'";
			$refquery 	= $this->db->query($refsql);
			$refresult 	= $refquery->row_array();
			$ref_id = $refresult['refid'];
			...
			$sql = "insert into affiliate (`refid`,`sid`,`purchaseamount`,`amount`,`createAt`) values ({$ref_id},{$uid},'{$money}','{$fee}',NOW())";
			$query = $this->db->query($sql);
			//-- affiliate earning end-------
	...
	public function delaff($id)
	{
		$id=$id[0];
		$sql = "delete from affiliate where id='{$id}'";
		return $query11 = $this->db->query($sql);
	}
	
	public function getAllSchoolPaymentTransaction($start,$search = '',$limit,$sortorder,$sort) {
		if($search == ''){
			$sql = "SELECT  * from school_payment_transaction order by school_id {$sortorder} ";
		}else{
			$sql = "SELECT  * from school_payment_transaction where MONTH(date_paid)={$search} order by school_id {$sortorder}";
		}
		$query 	= $this->db->query($sql);
		$result = false;
		if ($query->num_rows() > 0) {
	...
	public function del_school_payment_transaction($id)
	{
		$id=$id[0];
		$sql = "delete from school_payment_transaction where id='{$id}'";
		return $query11 = $this->db->query($sql);
	}
}
