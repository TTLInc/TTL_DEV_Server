...
class Angular_search_model extends TL_Model{
	...
	public function getResult()
	{
		...
		if($this->session->userdata('uid')){
			...
			if($school_id != '0'){
				$this->db->select('sc_flag');
				$this->db->from('profile');
				$this->db->where('uid', $this->session->userdata('uid'));
				$school_flag = $this->db->get()->row()->sc_flag;
					if($school_flag == '0'){
							$cnd = "";
							...
							$cnd = "";
							$orderBy = "`user`.`id` = $school_id DESC,`user`.`id` ASC,`readytotalk` DESC, `user`.`roleId` DESC, `nativeLanguage` ASC ";
							$order = "";			
							$cnd .= " and (`profile`.`pic` != '') and (`profile`.`BioGraphy` != '0') and `quarantine` != '1' and `hiddenRole` = '0'";
							$selectChkFreeSession = "";
							$setPrfHrate = ", `profile`.`hRate` "; 
