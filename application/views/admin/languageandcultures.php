<?php $this->layout->setLayoutData('content_for_layout_title','Language and Culture');?>
<?php $this->layout->setLayoutData('content_for_layout_links','<a href="'.base_url('admin/addLanguageAndCulture').'">Add New</a>');?>
<table class="data_table">
    ...
    <tbody>
    <?php $j = 0; foreach ($blogs as $key => $val) :?>
        <tr id="id_<?php echo $val['id'];?>">
            <td><input type="checkbox" name="ids[]" value="<?php echo $val['id'];?>" /></td>
            <td><?php echo $val['id'];?></td>
            <td><?php echo $val['title'];?></td>
            <td><?php echo isset($val['image']) ? basename($val['image']) : "";?></td>
			<td><?php echo $val['status'];?></td>
            <td>
            <a href="<?php echo base_url('admin/addLanguageAndCulture/'.$val['id']);?>">
...
function del(_id){
	if(window.confirm('Are you sure you want to delete this?')){
		$.post('<?php echo Base_url("admin/delLanguageAndCulture")?>',{id:_id},function(msg){
