	...
	public function auto_user()
	{
		...
		if(count($users)>0)
		{
			...
			foreach($users as $user)
			{
				...
				$final = '<span class="bold">'.ucfirst($str).'</span>';
	...
	**/
	...
	public function forward_message($uid,$inbId,$subject){
		...
		if($messageId != ''){
			...
			$message = $inbox['message'];
			$fromName = $ffname;
			$str = $ffname.' sends this message to you from TheTalkList:<br/><br/>';
			$str .= $message;
			$this->load->library('email');
			$this->email->mailtype = 'html';
			$this->email->from('no-reply@thetalklist.com',$fromName);
	...
	function studentmsg()
	{
		...
		if(count($Session)>0)
		{
			...
			for($i=0;$i<count($Session);$i++)
			{
				...
				$stuid=$Session[$i]['sid'];
				$mny=$Session[$i]['fee'];
				$sql12="update profile set money=money+'{$mny}',purchased_credits=purchased_credits+'{$mny}',frMoney=frMoney-'{$mny}' where uid='{$stuid}'";
				$query = $this->db->query($sql12);
				$Name=$Session[$i]['firstName'].'  '.$Session[$i]['lastName'];
				$date=date("Y-m-d", strtotime($Session[$i]['startTime']));
				$str .= str_replace('{$name}',$Name,nl2br($newslettertemplate['n10']));
...
public function sendBookNowMessage(){
	...
	$feeSql = "update profile set money=money-{$fee}, purchased_credits=purchased_credits-{$fee}, frMoney=frMoney+{$fee} where uid={$sid}";
	$query = $this->db->query($feeSql);
	...
	public function fblogin(){
        ...
        if($user!="") :      
			...
			if(empty($userdata)) : 
				...
				/*$formData['fb_gender'] = $user['gender'];		
				$formData['fb_birthday'] = $user['birthday'];		
				$formData['fb_location'] = $user['location'];*/
				$formData['universal_roleId'] = '0';
				$formData['free_session'] = 'y';
				$formData['is_eligible'] = '1';
				$formData['testAccount'] = '0';
