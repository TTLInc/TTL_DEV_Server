...
class User_model extends TL_Model{
	...
	*/
	public function register($userinfo) {
		...
		$sql.="username=?,email=?,password=?,roleId=?,free_session=?,timezone=?,is_eligible=?,testAccount=?,isconfirmedAccount=?,universal_roleId=?,facebook_id=?,reg_source=?";
		$query = $this->db->query($sql,array($userinfo['email'],$userinfo['email'],$userinfo['password'],$userinfo['roleId'], $userinfo['free_session'],$userinfo['timezone'],$userinfo['is_eligible'],$userinfo['testAccount'],$userinfo['isconfirmedAccount'],$userinfo['universal_roleId'],$userinfo['facebook_id'],$userinfo['reg_source']));
		/* End */
		return $this->db->insert_id();
	}
	...
	function checkdismiss($uid){
		$dismissforever = "SELECT dismissforever from user where id = '".$uid."'";
		$query = $this->db->query($dismissforever);
		if($query -> num_rows() > 0){
			$result = $query->row();
		}
		return $result;
