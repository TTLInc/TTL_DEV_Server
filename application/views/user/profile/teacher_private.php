...
<video src="<?php echo $videopath; ?>" poster="<?php echo $imagepath;?>"></video>-->
					...
					<?php if($profile['school_id'] == '0') {?>
							<div class="selection">
							<!--	<form method="post" action="<?php //echo site_url('user/tutor_scl'); ?>">  -->
									<table>
										<tr>
											<td style="padding:10px; font-size: 16px;">Select School :</td>
											<td style="padding:10px;">
												<select style="width:250px" class = "itemName" id="get_school" name="school" >	</select >
											</td>
											<td></td>
											<td><button onclick="search_school()" type="Submit" class="btn btn-default" style="background-color:#3399cc;  color:#fff">Select</button></td>
					...
					<?php } ?>
					<p id="msg" style="font-size: 12px; padding: 10px; display:none"></p>
                    <div class="mod">
...
<script>
   $('.itemName').select2({
		...
		allowClear: true,
        ajax: {
          url: '<?php echo base_url('user/search_school/'); ?>',
          dataType: 'json',
          delay: 250,
...
function search_school(){
	var sc = document.getElementById('get_school').value;
		$.ajax({
				url: "<?php echo base_url('user/tutor_scl/'); ?>",
				data: { school: sc},
				dataType: "html",
				type: "POST",
				success: function(data) {
					if(data == "Success"){
						//$("#get_school").select2("val", "");
						$('#get_school').select2('val',null);
						$('#msg').css('display','block');
						$("#msg").append("School has been successfully selected.");
