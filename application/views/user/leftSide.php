...
$arrVal 	= $this->lookup_model->getValue('890', $multi_lang);	$createprofile  	= $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('891', $multi_lang);	$photo  	= $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('892', $multi_lang);	$vgreeting  	= $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('630', $multi_lang);	$bgraphy 	= $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('1039', $multi_lang);    $YouMust   = $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('1062', $multi_lang);    $videoupload   = $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('443', $multi_lang);	$lUPLOAD_VIDEO_TIP   	= $arrVal[$multi_lang];

$arrVal 	= $this->lookup_model->getValue('1191', $multi_lang);	$affittoltip  	= $arrVal[$multi_lang];
$arrVal 	= $this->lookup_model->getValue('1228', $multi_lang);	$calOpening  	= $arrVal[$multi_lang];
?>
...
<?php //$this->layout->appendFile('javascript',"https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.17/jquery-ui.min.js"); ?>
...
window.setInterval(function(){
  //chkprofile();
  //added by haren to send message if student absent from session 
  var rid="<?php echo $this->session->userdata('roleId');?>";
  if(rid==0)
  {
	//sendRemaindeMessage();
  }
...
function sendRemaindeMessage()
{
	$.ajax({
		type:'POST',
		url:'<?php echo base_url('user/CheckForAbsent/');?>',
	...
	$.ajax({
		...
		success:function(msg){
			...
			if(json.success==true){	
				...
				$('#dialog').html('<?php echo "Thanks for completing your profile.  Your account is now visible.";?>');
			
			}
		else if(json.success=='pupload') {
			location.reload();
			}
			
			else if(json.success=='vupload') {
			//location.reload();
			}
			else if(json.success=='bupload')
			{
			location.reload();
			}
			else if(json.success=='calopen')
			{
			location.reload();
			}
			else{
...
$(".AffiTool").hover(function () {
			 $(this).append("<div class='tooltip-payment'><p><?php echo  $affittoltip; ?></p></div>");
				}, function () {
...
.AffiTool span{   float: left;
    text-align: justify;
    width: 100%;
    word-break: break-all;
    word-wrap: break-word;
}
.AffiTool {cursor: pointer;display: inline-block;width: 10px;line-height: 10px;font-size: 13px;font-weight: bold;border-radius: 4px;text-align: left;position: relative; float:left;width: 100% !important;
...
div.tooltip-payment p {margin: 10px;color: White;   line-height: 18px;}
</style>
...
/* Blink Favicon code end */
	...
	function getUnreadMessagesReal(){
		...
		msgTimer = setTimeout('getUnreadMessagesReal()',30000);
	}
...
$( document ).ready(function() {
		$('#vupld').click(function(){  
			...
			//$('input[accept=video/*]').css('display','block');
	   ...
	   var button12 = $('#new_upload_video_lnk_true'), interval1;
		...
		action: '<?php echo Base_url('user/profile_video');?>',
			...
			type:'video/*,video/mp4,video/3gpp,video/rm,video/avi,video/wma,video/rmvb,video/wmv,video/mov,video/MOV',
			 ...
			 /*type :' [
            {title : "Video files",extensions: "*,3gpp"}
        ]',*/
			...
			onSubmit : function(file, ext){
				...
				if(ext == 'mp4'|| ext == 'MOV' || ext == 'mov' || ext == 'avi' || ext == 'flv'|| ext == 'rm'|| ext == 'wma'|| ext == 'rmvb'|| ext == '3gp'|| ext == 'wmv'){
				}else{
			...
			onComplete: function(file, response){
				...
				//convertVideo(response.videoname); // Pass Video Name ex: 2014/02/vfasdfsdfdsfsd.mov
				
				alert('Your video has been uploaded successfully, After video reformat process, it will be ready in 1 hour.');
				$.unblockUI();
				return false;
				/*$.unblockUI();
...
<div id="sendMessageView" class="sendMessageView" style="display:none;"></div>
	<div class="content_side fl" style="float:left; width:320px;">
		<div class="slide_inner">
			...
			<div  <?php if($own) {?>	id="ProfImg" style="background:none !important;cursor:pointer" <?php } else {?> style="background:none !important;" <?php } ?> class="user_header posR">
				<?php 
				if(!$profile['pic']){
						$iheight = '245';
					}else{
				...
				<a href="#" style="display:none;" class="upload_hdpic"  id="profile_image_upload"><?php echo $lUPLOAD_PICTURE;?></a>
                <?php if($profile['roleId'] == 2  || $profile['roleId'] == 1  || $profile['roleId'] == 3): ?>
				<div class="ratings_score_b lft-img-rt" id="">
				<span class="ratings_score_yellow star<?php echo $profile['avgRate'];?>">
				</span>
				</div>
				<?php endif; ?>
			...
			</div> <?php if(!$own AND $profile['fbshare'] != '0') {?><div class="fb-share-button forleftside" data-href="<?php echo base_url("user/profile/uid/".$profile['uid']);?>" data-layout="button"></div><?php } ?>
			<?php //echo "<pre>"; print_r($profile);die;?>
			
			<input type="hidden" name="profilehpic" id="profilehpic" value="<?php echo $profile['pic'] ?>" />
			<div class="user_info">
            	<?php if(!$login):?>
				<div class="signup_btn"><a href='<?php echo base_url('user/signup/?g=tutorpro&uid='.$profile['uid']);?>'>Book Me</a></div>
				<?php endif;?>
				<div><?php if(!$own):?>	<a href='<?php echo base_url('user/signup/?g=tutorpro&uid='.$profile['uid']);?>'><?php echo mailAptBtn($chkfreesession,$profile['uid']); ?></a><?php endif ;?></div>
				<span  style="top:30px;font-size:20px;font-weight:bold;" class="tutr-nm"><?php echo $profile['firstName'];?></span>
		
					<?php  if($profile['roleId'] == 5 || $profile['roleId'] == 4){?> <p style="font-size:11px;" class="AffiTool"><span><?php  echo str_replace(" ","-",$encode);?></span></p> <?php }?> 

						<?php if($profile['roleId'] == 2): ?><br><span style="bottom:30px;font-size:20px;font-weight:300;" class="tut-infonw"><?php echo $lsilvertutor;?></span><?php endif;?>
						<?php if($profile['roleId'] == 3): ?><br><span style="bottom:30px;font-size:20px;font-weight:300;" class="tut-infonw"><?php echo $lgoldtutor; ?></span><?php endif;?>
						<?php if($profile['roleId'] == 0): ?><br><span style="bottom:30px;font-size:20px;font-weight:300;" class="tut-infonw"><?php echo $lstudenttalkist; ?></span><?php endif;?>
						<?php if($profile['roleId'] == 1): ?><br><span style="bottom:30px;font-size:20px;font-weight:300;" class="tut-infonw"><?php echo $lbronzetutor;?> </span><?php endif;?>
						<!-- Added by Ilyas-->
						<?php if(!$own):?>
                        <?php
						...
						?>
                        <?php
						if($this->session->userdata('roleId')!="" && $this->session->userdata('roleId')<4 && $current_uri4 != $this->session->userdata('uid') && $this->uri->segment(2) == 'profile'){?>
                        <div class="left-action-section nw-tutr-btn">
						
                        
                        <a class="icon-msg icon-popup icon1"  onclick="sendBeepBoxMessage(<?php echo $current_uri4 ;?>)">&nbsp;<span class="classic"><?php echo $smessage;?></span></a>                      
						<?php if($profile['readytotalk']==1){?>
                                <a class="icon-tk-nw icon-popup icon3-red icon2 bkclass" onClick="bookNow(<?php echo $current_uri4 ;?>,'<?php echo $profile['firstName'];?>','<?php echo $profile['school_id'];?>','<?php echo $profile['hRate'];?>')">&nbsp;<span class="classic"><?php echo $talk; ?></span></a>
                        <?php } else {?>
                            <a class="icon-tk-nw icon-popup icon3-gray icon2"  >&nbsp;<span class="classic"><?php echo $talknotavail; ?></span></a>
                         <?php } ?>
                            <a class="icon-fav icon-popup icon3" onClick="addToPotential(<?php echo $current_uri4 ;?>)">&nbsp;<span class="classic"><?php echo $favoutite; ?></span></a>
                            </div>
                            <?php }?><?php endif; ?>
                            <!-- End By Ilyas --> 
						<?php if($own):?>	
						<?php /*if($profile['roleId'] == 4) {?>
							<!--<a class="owner-settings" style="margin-top:-45px;" href="<?php echo base_url('user/myInfo');?>"><img src="<?php echo base_url('images/setting-icon.png');?>" alt="<?php echo $lsettings; ?>"></a>-->
							<a class="owner-settings" href="<?php echo base_url('user/myInfo');?>" title="<?php echo $lsettings; ?>"><span class="setting-icon"></span></a>
						<?php }
						elseif($profile['roleId'] == 5)
						{?>
							<a class="owner-settings" href="<?php echo base_url('user/AffiliateInfo');?>" title="<?php echo $lsettings; ?>"><span class="setting-icon"></span></a>
							<!--<a class="owner-settings" style="margin-top:-45px;" href="<?php echo base_url('user/AffiliateInfo');?>"><img src="<?php echo base_url('images/setting-icon.png');?>" alt="Settings"><?php //echo $lsettings; ?></a>-->
						<?php }else {?>
							<!--<a class="owner-settings" href="<?php echo base_url('user/changeInfo');?>"><img src="<?php echo base_url('images/setting-icon.png');?>" alt="Settings"><?php //echo $change_contact_info;?><?php //echo $lsettings; ?></a>-->
							<a class="owner-settings" title="<?php echo $lsettings; ?>" href="<?php echo base_url('user/changeInfo');?>"><span class="setting-icon"></span></a>
						<?php }*/?>	
							...
							<div class="bknwhwr"><p class="defaultP"><input type="checkbox" name="readytotalk" id="readytotalk" value="1" <?php if(@$profile['readytotalk']=='1'){echo 'checked';} ?> /></p></div>
							<?php endif; ?>
						<?php endif; ?>
					...
					<!-- for tutor profile haren -->
				 
				  <?php 
				  if($otheruser != 1 AND $profile['uid'] == $this->session->userdata['uid']){
				  if($this->session->userdata['roleId'] == 1 || $this->session->userdata['roleId'] == 2 || $this->session->userdata['roleId'] == 3) {?>
							<?php if($pinfo['pic']!='' && $pinfo['vedio']!='' && $pinfo['personal']!='' && $pinfo['professional']!='' && $pinfo['academic']!='' && $openedslot['oslot'] > 0){?>
							
							<?php } else {?>
							<span style="font-size:15px;"><?php echo $createprofile;?> <br>
							<div class="blog-hight-light">
							<span class=""><a  id="imupld"  href="#"  <?php if($pinfo['pic']!=''){ ?>class="golden-btn-icon bknw"<?php }else {?>class="golden-btn-wicon bknw"<?php } ?> style="cursor: default;margin-bottom:5px;margin-top:5px" ><?php echo "1". " ".$photo;?></a></span><br />
							<span class="vidup"><a  id="vupld" href="#" <?php if($pinfo['vedio']!=''){ ?>class="golden-btn-icon bknw"<?php }else {?>class="golden-btn-wicon bknw"<?php } ?> style="cursor: default;width:116px;margin-bottom:5px" ><?php echo "2"." ".$vgreeting;?></a></span><br />
							<span class=""><a href="<?php echo base_url('user/profile');?>" <?php if($pinfo['personal'] !='' && $pinfo['professional'] !='' && $pinfo['academic'] !=''){ ?>class="golden-btn-icon bknw"<?php }else {?>class="golden-btn-wicon bknw"<?php } ?> style="cursor: default;width:92px;margin-bottom:5px" ><?php echo "3". " ".$bgraphy;?></a></span><br />
							<span class=""><a href="<?php echo base_url('user/calendar');?>" <?php if($openedslot['oslot'] > 0){ ?>class="golden-btn-icon bknw"<?php }else {?>class="golden-btn-wicon bknw"<?php } ?> style="cursor: default;width:92px;margin-bottom:5px" ><?php echo "4". " ".$calOpening;?></a></span><br /> </div>
							</span>
							<?php }?>
					<?php }
					}
					...
					<!-- end tutor profile -->
						...
						<?php if($adcon): ?>
                                        ...
                                        <div class="adverties" style="overflow:hidden; width:260px;height:150px; float:left;border-bottom: 4px solid #D7BC4D;padding:0px 0px 30px 0px;">
                                            ...
                                            <ul class="bjqs">
                                                ...
                                                <div class="panel">
													<div class="box-cont over-hid">
														 ...
														 <?php if($ad['link'] != '') {?>
														<div class="">
															<a target="_blank" href="<?php echo 'https://'.$ad['link']; ?>">
																
																<img onclick='return click_count(<?php echo $ad['advertisementid'];?>)' style="width:180px;height:150px;padding-top:6px;margin-left:45px;"  src="<?php echo base_url('uploads/images/ad/'.$ad["source"]); ?>" alt="01"/>
															</a>
														</div>
														<?php } else { ?>
														<div class="">
															
															<img src="<?php echo base_url('uploads/images/ad/'.$ad["source"]); ?>" alt="01" style="width:180px;height:150px;padding-top:6px;margin-left:45px;"/>
														</div>
														<?php } ?>
													</div>
                                                </div>
					...
					<?php
						...
						<?php if($adcon1): ?>
                         <div class="adverties" style="overflow:hidden; width:260px;height:160px; margin-top:1px !important;float:left;border-bottom: 4px solid #D7BC4D;padding:20px 0px 20px 0px;">
                                            ...
                                            <ul class="bjqs">
                                                ...
                                                <div class="panel">
													<div class="box-cont over-hid">
														 ...
														 <?php if($ad1['link'] != '') {?>
														<div class="">
															<a target="_blank" href="<?php echo 'https://'.$ad1['link']; ?>">
																
																<img onclick='return click_count(<?php echo $ad1['advertisementid'];?>)' style="width:180px;height:150px;padding-top:6px;margin-left:45px;"  src="<?php echo base_url('uploads/images/ad/'.$ad1["source"]); ?>" alt="01"/>
															</a>
														</div>
														<?php } else { ?>
														<div class="">
															
															<img style="width:180px;height:160px;padding-top:6px;margin-left:45px;" src="<?php echo base_url('uploads/images/ad/'.$ad1["source"]); ?>" alt="01"/>
														</div>
														<?php } ?>
													</div>
                                                </div>
                                            ...
                                            <ul class="bjqs">
                                                ...
                                                <div class="panel">
													<div class="box-cont over-hid">
														 ...
														 <?php if($ad2['link'] != '') {?>
														<div class="">
															<a target="_blank" href="<?php echo 'https://'.$ad2['link']; ?>">
																
																<img onclick='return click_count(<?php echo $ad2['advertisementid'];?>)' style="width:180px;height:150px;padding-top:6px;margin-left:45px;"  src="<?php echo base_url('uploads/images/ad/'.$ad2["source"]); ?>" alt="01"/>
															</a>
														</div>
														<?php } else { ?>
														<div class="">
															
															<img src="<?php echo base_url('uploads/images/ad/'.$ad1["source"]); ?>" style="width:180px;height:150px;padding-top:6px;margin-left:45px;" alt="01"/>
														</div>
														<?php } ?>
													</div>
                                                </div>
 ...
 <?php
		...
		$(function(){
			...
			$('#fbshare').click(function(){
				$('#fbshare').each(function(){
					var fbshare = 0;
					if(this.checked){fbshare = 1;}
					var ddataStringChecked = "fbshare="+fbshare;
					$.ajax({
						url: "<?php echo base_url();?>user/fbshareUpdate",
						type: 'POST',
						data: ddataStringChecked,
						dataType: 'json',
						cache: false,
						success: function (msg){
							if(msg.status == 'success'){
								if(fbshare == 1){
									alert('Facebook share icon is visible on your profile to share it on Facebook from other users.');
								}else{
									alert('Facebook share icon is invisible on your profile to share it on Facebook from other users.');
								}
							}
						}
					});
		...
		$(document).ready(function () {
			  ...
			  $("span.bknwhwr").hover(function () {
			    ...
			    $("span.vidup").hover(function () {
				$(this).append('<div class="tooltip3"><p><?php echo  $lUPLOAD_VIDEO_TIP; ?> </p></div>');
			  }, function () {
				$("div.tooltip3").remove();
			  });
...
<!--<div class="container-narrow" id='DynamicPopup' style='display:none'>
<div class="masthead">
...
<hr>
<div class="jumbotron">
<h1 data-step="1" id="step1">Programmatic</h1>
<p class="lead">In this example we are going to define steps with JSON configuration.</p>
<a class="btn btn-large btn-success" href=" " onclick='startIntro()'  id='myviewpoint'>S</a>
</div>
<hr>
<div class="row-fluid marketing">
<div data-step="2" id="step2" class="" data-position='left'>
<h4>Section One</h4>
...
<div data-step="3" id="step3" class="span6" data-position='left'>
<h4>Section Four</h4>
<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis mollis augue a neque cursus ac blandit orci faucibus. Phasellus nec metus purus.</p>
<h4>Section Five</h4>
<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis mollis augue a neque cursus ac blandit orci faucibus. Phasellus nec metus purus.</p>
...
<hr>
	...
	<?php //include dirname(__FILE__).'/multipage.php';?>
...
span.vidup{cursor: pointer;display: inline-block;width: 98px;height: auto;line-height: 16px;color: White;font-size: 13px;font-weight: bold;border-radius: 8px;position: relative;}

div.tooltip {background-color: #3399cc;color: White;position: absolute;left: 35px;top: -15px;z-index: 1000000;width: 250px;border-radius: 5px;}
div.tooltip:before {border-color: transparent #3399cc transparent transparent;border-right: 6px solid #3399cc;border-style: solid;border-width: 6px 6px 6px 0px;content: "";display: block;height: 0;width: 0;line-height: 0;position: absolute;top: 40%;left: -6px;}
...
.bknw{ width:65px; padding:0px;}
span.bknwhwr{cursor: pointer;display: inline-block;width:68px;height: 16px;line-height: 16px;color: White;font-size: 13px;font-weight: bold;border-radius: 8px;text-align: center;position: relative;}
div.tooltip3 {background-color: #3399cc;color: White;position: absolute;left:50px !important;top: -45px !important;z-index: 1000000;width: 250px;border-radius: 5px;}
div.tooltip3:before {border-color: transparent #3399cc transparent transparent;border-right: 6px solid #3399cc;border-style: solid;border-width: 6px 6px 6px 0px;content: "";display: block;height: 0;width: 0;line-height: 0;position: absolute;top: 40%;left: -6px;}
