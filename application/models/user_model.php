...
class User_model extends TL_Model{
	...
	public function getSchoolPaymentNoteCount(){
		$result = array();
		$length = 20;
		$sql = "SELECT * FROM school_payment_note";
		$query = $this->db->query($sql);
		$result['count'] = 0;
		if ($query->num_rows() > 0) {
			$result = $query->row_array();
	...
	*/
	...
	public function getnameByUid($uid){
		$result = array();
		$sql = "select firstName,lastName,pic from profile where uid={$uid} limit 1";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0) {
			$result = $query->row_array();
		}
	...
	public function getAllAdmin($start = 0,$sort,$sortorder,$length = 20){
		$result = array();
		$sql = "select u.*, CONCAT(p.firstName, ' ',p.lastName) as usernm, p.lms_complete,p.money,p.purchased_credits,p.coupon_credits,p.affiliate_credits,p.earned_credits from user as u,profile as p where u.id = p.uid order by {$sort} {$sortorder} limit {$start} , {$length}";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
		}
	...
	public function getAllAdminBySearch($start = 0,$search = '',$sort,$sortorder,$length = 20){
		$result = array();
		$sql = "select u.*, (select expDate from pay c where uid = u.id order by id desc limit 0, 1) as expDate, CONCAT(p.firstName, ' ',p.lastName) as usernm, p.lms_complete,p.money,p.purchased_credits,p.coupon_credits,p.affiliate_credits,p.earned_credits from user as u,profile as p where u.id = p.uid and (u.id like '{$search}' OR u.email like '{$search}' OR p.firstname like '%{$search}%' OR p.lastName like '%{$search}%' ) order by {$sort} {$sortorder} limit {$start} , {$length}";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
		}
	...
	{	
		...
		$sqlquery = "SELECT profile.firstName,profile.lastName,profile.school_id,profile.uid,user.roleId FROM profile INNER JOIN user ON profile.uid = user.id where user.roleid > 0 AND profile.school_id = ".$id;
	...
	public function GetSchoolEarning($sid)
	{
		...
		$this->db->where("month(createAt) = ".date('m'));
		$query = $this->db->get();  
		if ($query->num_rows() > 0) {
			$result= $query->result_array();
		}
	...
	public function affiSummary($search,$sort,$sortorder)
	{
		...
		if ($search == '') {
			 $sql= "select distinct user.email,user.id, profile.firstName, profile.lastName,profile.principle_name,profile.organizationName,user.add_time from user, profile where user.id= profile.uid and user.roleId=5 GROUP BY profile.uid order by {$sort} {$sortorder}";
		} else {
	...
	public function school_payment_addnote($data) {
		$sql	= "INSERT INTO school_payment_note set  ";
		$sql	.= "dispute_id=?,school_id=?,note=?,role=?,add_date=?";
		$query	= $this->db->query($sql,array($data['dispute_id'],$data['school_id'],$data['note'],$data['role'],$data['date']));
		return  $this->db->insert_id();
	}
	...
	public function GetAllSchoolPaymentNotes($start,$sort,$sortorder){
		$sid = $this->input->_request('id');
		$start = 0;
		$result = array();
		$length = 20;
		$sql = "SELECT * FROM school_payment_note WHERE school_id={$sid} order by {$sort} {$sortorder}";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0) {
			$result = $query->result_array();
		}
	...
	public function getsesstioncostnew($userid,$configvalues,$tutorid)
	{
		...
		if ($isnew != "0" && $res['free_session'] == 'y') {
			...
			if ($stuid !='') {
				...
				if ($refid > 0) {
					...
					$q = "Select hRate,school_id,firstName from profile where uid = ".$tutorid;
					$query = $this->db->query($q);
					$resultmoney = 0;
					if ($query->num_rows() > 0) {
						$result = $query->result_array();
		...
		return array("isnew"=>$isnew,"tutorcost"=>$tutorcost,"schooltutorcost"=>$schooltutorcost,"affiliate"=>$affiliate,"pbalance"=>$pbalance,"school_id"=>$schoolid,"firstName"=>$firstName,"uid"=>$tutorid);
	}
	...
	{
		$sql = "SELECT `user_id`,`id`,`amount`, DATEDIFF( now( ), `payment_date` ) AS `datediff` FROM `transaction` WHERE `payment_status`='Paid' and `status`='Pending' and `type`='Cashout' HAVING `datediff` >= 1 order by `payment_date` DESC";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}
	...
	public function fnGetReadyToTalkNowUsers()
	{
		...
		if($school_id == '0'){
			...
			//$result = array('message'=>'No record found.');
			return $result;
		}else{
		
			$cnd = " and `user`.`readytotalk` ='1' and (`profile`.`pic` != '') and `quarantine` != '1' and `user`.`id`!='".$uid."' and `user`.`hiddenRole` !='1' ";
			//$sql = "SELECT `user`.`id`, `profile`.`uid`, `profile`.`firstName`, `profile`.`lastName`, `profile`.`pic`, `user`.`readytotalk`,`profile`.`school_id` $setPrfHrate  from `user` JOIN `profile` ON  `user`.`id` = `profile`.`uid` WHERE `user`.`roleId` IN (1,2,3) $cnd ";
			$sql = "SELECT `user`.`id`, `profile`.`uid`, `profile`.`firstName`, `profile`.`lastName`, `profile`.`pic`, `user`.`readytotalk`,`profile`.`school_id` $setPrfHrate  from `user` JOIN `profile` ON  `user`.`id` = `profile`.`uid` WHERE `user`.`roleId` IN (1,2,3) and `profile`.`school_id` = $school_id";
			$query = $this->db->query($sql);
			$result = array();
			if ($query->num_rows() > 0) {
				$result = $query->result_array();
	...
	function mode_views_sql(){
		$VideoQuery = "SELECT views from lessons where id = 4";
		$query = $this->db->query($VideoQuery);
		if($query -> num_rows() > 0){
			$result = $query->row();
		}
	}
	function checktalknowlight($uid){
		$sql 	= "SELECT `readytotalk`, `is_login` FROM `user`  WHERE `id` ='".$uid."'";
		$query 	= $this->db->query($sql);
		$result = array();
		if ($query->num_rows() > 0) {
			$result = $query->row();
	...
	function getAffiScid($uid){
		...
		$query = $this->db->get()->row()->flag;
		
		if($query == 0){
			
			$sql_update = "UPDATE promotion_links set redeemed = 'yes' , flag = '1' WHERE code = '{$uid}' ";
			...
			if ($query_inner->num_rows() > 0) {
				$result = $query_inner->row_array();
				}
			
			return $result;
		}else{
			$result = array('school_id'=>0,'affiliate_id'=>0);
			return $result;
		}
