<?php
$this->layout->setLayoutData('content_for_layout_title','Attempted Sessions');
$currentUrl = base_url('admin/Attempted_Session');
if($dsearch =='')
{
 $dsearch=date('m/Y');
}

$this->layout->setLayoutData('content_for_layout_links','<div style="float:left;font-weight:bold;"><input style="width:22%;padding:4px;float:left;margin-right: 5px;" class="adm_box1" type="text" placeholder="select date" value="'.$dsearch.'" name="dsearch" id="events_widget" /><input style="width:23%;padding:4px;float:left;margin-right: 5px;" class="adm_box1" type="text" value="'.$search.'" name="search" id="search"  /><a href="javascript:void(0);" class="button submit-search" style="height:17px;line-height:19px;">Search</a> </div>');
echo $this->session->userdata('searchdisputes');
$this->layout->appendFile('javascript',"js/jquery.mtz.monthpicker.js");
?>
			...
			/*if(@$val['session_type'] != 'free'){*/?>
				<tr id="id_<?php echo $val['id'];?>">
					...
					<td>
						...
						echo date('Y-m-d H:i:s',$currentDtStr);
						?>
					</td>
					<td>
						<?php
						...
						// Updated By Ilyas 
						/*if (@$val['sconnect']=='N' and @$val['tconnect']=='N' and @$val['endTime'] < $edate) {
							if (@$val['type'] == "now") {
								echo "Now Aborted";
							} else if (@$val['type'] == "general" and @$val['Booking']=='Requested') {
								echo "Req Aborted";
							} else if (@$val['type'] == "general" and @$val['Booking']=='Booked') {
								echo "Sched Aborted";
							}
						} else if (@$val['sconnect']=='N' and @$val['tconnect']=='Y' and $val['s_attend'] == 0 and @$val['endTime'] < $edate) {
							if (@$val['type'] == "now") {
								echo "Now Aborted";
							} else if (@$val['type'] == "general" and @$val['Booking']=='Requested') {
								echo "Req Aborted";
						...
						} else if (@$val['sconnect']=='Y' and @$val['tconnect']=='N' and $val['s_attend'] == 0 and @$val['endTime'] < $edate) {
							if (@$val['type'] == "now") {
								echo "Now Aborted";
							} else if (@$val['type'] == "general" and @$val['Booking']=='Requested') {
								echo "Req Aborted";
						...
						/*
						if (@$val['Intent'] >=2 AND @$val['session_type'] != 'free') {
							if (@$val['type'] == "now") {
								echo "Now Aborted";
							} else if (@$val['type'] == "general" and @$val['Booking']=='Requested') {
								echo "Req Aborted";
					...
					<td><?php if(@$val['session_type'] == 'free'){ echo "Yes";} else { echo "No";}?></td>
...
setTimeout('showmenu("financial",2)',1000);
function del(_id){
...
$(function(){
	...
	$(".submit-search").click(function() {
		...
		$form = $("<form id='sessioncnt' method='post' action='<?php echo base_url()."admin/Attempted_Session/";?>'></form>");
		if (search !='' || dsearch == '') {
