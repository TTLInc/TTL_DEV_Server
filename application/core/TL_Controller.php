...
class TL_Controller extends CI_Controller {
	...
	function __construct(){
		...
		// Replace Segments Value
		$segs = $this->uri->segment_array();
		if(in_array(@$this->input->uri->segments[1],$this->config->item('lang_uri_abbr'))) {
			$this->input->uri->segments[0] = $this->input->uri->segments[1];
			foreach ($segs as $key=>$segment)
			{
				if($key < sizeof($segs)) {
					$this->input->uri->segments[$key] = $this->input->uri->segments[$key+1];
				}
			}
		}
		$this->layout->setlayoutData(
			array(
				'title_for_layout' =>mysql_real_escape_string('TheTalkList: Real people teaching real language'),
