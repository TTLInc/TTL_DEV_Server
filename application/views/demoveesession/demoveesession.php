<?php
$vjsurl	    		=	base_url().'js/vjs/';
$uploaddir	    	=	FCPATH.'vedio/';
$uploadurl	    	=	base_url().'vedio/';
...
function get_video_orientation($video_path) {
	...
	$cmd 			= '/usr/bin/ffprobe  -i ' . $video_path . ' -show_streams 2>&1';
...
?>
<script id="jwplayer_script" type='text/javascript' src='<?php echo $vjsurl;?>jwplayer/jwplayer.js'></script>
<link href="http://vjs.zencdn.net/4.2/video-js.css" rel="stylesheet">
<script id="vjs_script" src="http://vjs.zencdn.net/4.2/video.js"></script>				
<style type="text/css">
* {margin: 0;padding: 0;}
body {font-size: 62.5%;font-family: Helvetica, sans-serif;background: url(<?php echo $vjsurl;?>images/stripe.png) repeat;}
			...
			<div id="contact-area">
				<?php
						if ($_FILES["file"]["error"] > 0){
							...
							move_uploaded_file($_FILES["file"]["tmp_name"], $uploaddir."" . $uploaded_video .''.$_FILES["file"]["name"]);
							$uploaded2 				= "" . $uploaded_video .''.$_FILES["file"]["name"];
							$uploaded 				= $uploaddir."" . $uploaded_video .''.$_FILES["file"]["name"];
						}
					   //$uploaded = 'test.avi';
				?>
				<?php	if($uploaded != ""){	 ?>
				<?php
						...
						$image 						= 	$vjsurl.'black_background.jpg';
						...
						if($i){
							...
							if (($ext=="flv") || ($ext=="3gp") || ($ext=="rmvb")) {
								...
								$filenameImage = $uploadurl.''.rand().'.jpg';
						...
						<?php //if($current_browser != 'msie'){ ?>
							<div id="vjs_div" style="display:none;">
								<video id="example_video_3" class="video-js vjs-default-skin vjs-big-play-centered" controls preload="auto" width="505" height="287"  data-setup='{}' poster='<?php echo $uploadurl;?><?php echo $filenameImage;?>'>
								<source src="<?php echo $uploadurl;?><?php echo $uploaded2;?>" type="video/mp4"/>
								</video>
							</div>
						<?php //} ?>
...
<script type='text/javascript'>
	...
	var output_file_full 	= '<?php echo $uploaded2;?>';
	...
	if(userAgent == 'msie'  && output_file_full != ''){
		...
		jwplayer('container').setup({
			flashplayer: '<?php echo $vjsurl;?>jwplayer/player.swf',
			file:'<?php echo $baseurl;?><?php echo $uploaded2;?>',
		...
		if(output_file_full != ''){
			...
			videojs.options.flash.swf = "<?php echo $vjsurl;?>video-js.swf";
