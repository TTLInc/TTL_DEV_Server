...
<div class="exp-btn">
<a onclick="ExportMemberpay();" href="javascript:void(0);">Export</a>
</div>
<?php 
if($dsearch =='')
{
...
<table class="data_table">
        ...
        <tr>
            ...
            <th class="rw5">
				<div class="rw5">
					...
					<a href="<?php echo $currentUrl.'/sortorder/desc/sort/role'.'/limit/'.$limit.'/page/'.$page; ?>"><span class="css_right ui-icon ui-icon-triangle-1-n"></span></a>
					<?php else: ?>
					<a href="<?php echo $currentUrl.'/sortorder/asc/sort/role'.'/limit/'.$limit.'/page/'.$page; ?>"><span class="css_right ui-icon ui-icon-triangle-1-s"></span></a>
					<?php endif;?>
				</div>
			</th>
	...
	foreach ($users as $key => $val) :?>
            <td class="rw1"><input type="checkbox" name="ids[]" value="<?php echo $val['id'];?>" /></td>
            <td class="rw2"><div class="rw2"><?php echo $val['id'];?></div></td>
            <td class="rw3"><div class="rw3"><?php echo $val['usernm'];?></div></td>
            
			<td class="rw5"<div class="rw5"><?php echo @$val['role'];?></div></td>
			...
			<?php 
			echo $val['BigBal']=number_format($val['opBal'],2,'.','');
			?>
			</div></td>
			<td class="rw10"><div class="rw10">
			<?php echo $val['addBal']=number_format($val['addBal'],2,'.','');?>
			</div></td>
			<td class="rw10"><div class="rw10">
			<?php echo $val['subBal']=number_format($val['subBal'],2,'.','');?>
			</div></td>
			<td class="rw10"><div class="rw10">
			<?php echo $val['endBal']=number_format($val['endBal'],2,'.','');?>
			</div></td>
			<td class="rw10"><div class="rw10">
			<?php 
			echo ($val['curBal']>0) ? $val['curBal'] : "0.00";
			?>
			</div></td>
            <td class="rw10"><div class="rw10">
...
setTimeout('showmenu("financial",5)',1000);
function del(_id){
...
<script>
function ExportMemberpay()
{
	var month = $('#events_widget').val();
	var actionPage = '<?php echo base_url().'admin/ExportMemberpay' ?>';
	$form = $("<form id='DispCsv' method='post' action='"+actionPage+"'></form>");
	$form.append('<input type="hidden" name="month" id="month" value="'+month+'" />');
	$('body').append($form);
	$('#DispCsv').submit();
}
</script>
