...
class User_model extends TL_Model{
	...
	*/
	...
	public function fnGetPendingCashout() 
	{
		...
		//$sql = "SELECT `user_id`,`id`,`amount`, DATEDIFF( now( ), `payment_date` ) AS `datediff` FROM `transaction` WHERE `payment_status`='Paid' and `status`='Pending' and `type`='Cashout' HAVING `datediff` < 2 order by `payment_date` DESC";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}
