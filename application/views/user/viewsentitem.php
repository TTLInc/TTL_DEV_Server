...
<script>
function cancelInfo(){
	 window.location.href="<?php echo base_url('user/sent');?>";
 ...
 <div class="baseBox baseBoxBg clearfix">
        <div class="content_main fr">
			...
			<?php //echo $linkType;?>
                ...
                <div id="student_prof_Wp">
                    <div class="mod">
                        <div class="hd">
                            <div class="pro_tle tle"><h3> <?php echo $beepbox;?></h3></div>
                        </div>
                        ...
                        <div class="bd">
                            <div class="inbox_mod">
                                ...
                                <?php if($message['attach'] !=''){?>	<br><label><?php echo ""; ?><img style="width:14px;" src="<?php echo base_url('images/clip.png');?>"></label><span id="message"><span style="width:14px;cursor:pointer" onclick="download(<?php echo $message['sentbox_id'];?>)"  /><?php echo $message['attach']."&nbsp;(".$Size.")";?></span><?php } ?></span>
                                </div>
                                <div class="inbox_list inbox_tle">
									<label><?php echo $Too;?>:</label>
									 
									<span id="from_firstname"><?php  echo @$message['firstName']. '' .@$message['uid'];?></span>
									<input type="hidden" id="from" value="<?php echo $message['username'];?>" />
									<input type="hidden" id="uid" value="<?php echo $message['to'];?>" />
									 
                                	
                                </div>
                                <span id="email" style="display:none;"><?php echo $message['from'];?></span>
								...
								<input type="hidden" id="messageid" value="<?php echo $message['InboxId'];?>" />
                            </div>
                        </div>
                    </div>
                </div>
	...
	<script>
	var showId = '<?php echo $message["sentbox_id"];?>';
	function prev(){		
		$('#dialog').html('<?php echo $waiting; ?>');
		$('#dialog').dialog({modal:true});
		var _data = {id:showId,type:'prev'};
		$.post('<?php echo base_url("user/MessageSentPrev");?>',_data,function(msg){
	...
	function next(){
		...
		$.post('<?php echo base_url("user/sentNext");?>',_data,function(msg){
			if (String == msg.constructor) {
				eval ('var json = ' + msg);
			} else {
				var json = msg;
	...
	function delDo(){ 
		...
		$.post('<?php echo base_url("user/DeleteSentItem");?>',_data,function(msg){
			...
			if(json.status){
				...
				$('#dialog').dialog({modal:true});
				document.location.href = '<?php echo base_url('user/sent');?>';
				//_tr.remove();
			}
			else{				
				$('#dialog').html(json.msg);
	...
	function forward(){
		document.location.href = '<?php echo base_url('user/SendMessage');?>/mid/'+showId;
	}
	$(function(){
