...
$arrVal = $this->lookup_model->getValue('380', $multi_lang);
$vclicktutorsession = $arrVal[$multi_lang];

$arrVal = $this->lookup_model->getValue('381', $multi_lang);
...
$languageArray = array(
        ...
        'closed'=>$closed,'close'=>$close,'open'=>$open,'opened'=>$opened,
        ...
        'successbooking'=>$successbooking,'nopermission'=>$nopermission,'firstlogin'=>$firstlogin,'enoughmoney'=>$enoughmoney,'speakLeval'=>$speakLeval,'youMustHave'=>$YouMustHave
        );
//echo "test";
...
echo "<pre>";*/
/*if($this->session->user('roleId') == '1'){
	echo "test";
}
exit;*/
?>
<script type="text/javascript" src="http://wcs.naver.net/wcslog.js"> </script>
<div id="divcourseselection" style="display:none;">This is Selection</div>
 <div class="baseBox baseBoxBg clearfix">
                         ...
                         <div class="content tle rating">
							  ...
							  <div class="cl-bking">
                              	<span>
									...
									?>
                                	<a   style="cursor:pointer;" onClick="bookNow(<?php echo $current_uri4 ;?>);"><img src="<?php echo site_url('images/pop-img-hst.png')?>"/></a>
									<?php } else {?>
									<img src="<?php echo site_url('images/tlk-nw-icon-hv.png')?>" />
									
									<?php }?>
                                </span>
                                <p><?php echo $TalkNowa;?></p>	
                              </div>
                              <div class="cl-tmlot">
                              	<span><a><?php echo $Books;?></a></span>
                              	<p><?php echo $OpenSlot;?></p>
                              </div>
                              <div class="cl-resession">
                              	<span><a><?php echo $Requests;?></a></span>
                                    ...
                                    <div class="selectboxtest">
                                        <form>
                                            <select name="sspeakinglevel" id="sspeakinglevel">
												...
												<option value="Beginner"><?php echo $lBEGINNER;?></option>
                                                <option value="Intermediate"><?php echo $lINTERMEDIATE;?></option>
                                                <option value="Advanced"><?php echo $lADVANCED;?></option>
                                            </select>
											...
											<input type="hidden" id="UserType" name="UserType" value="<?php echo $this->session->userdata['roleId'];?>"> 
	...
	<div id="calendarEventDialog" class="er_arr" style="display:none">
		...
		<span class="payment" style="float:right; margin:-3px 9px 0 0;"><img class="speakingLevelToolTip" src="<?php echo base_url('images/arrow.png');?>" alt="ed"></span>
		</span>
		</div>
		<!--R&D@Oct-10-2013 : IE popup message -->
		...
		$('.blackRadiusBtn','#calendarEventDialog').click(function(){			
			...
			$.post('<?php echo Base_url('user/checkClass/uid/'.$profile["uid"]);?>',_data,function(msg){
				...
				//else if(json.timeCheck24)
				{
					...
					//alert(_showstr);
					_showstr += '<?php echo $lcredit ?> '+parseFloat(json.cost).toFixed(2)+'<?php echo $lyour ?>. \r\n<?php echo $lOK_TO_CONFIRM;?>\r\n';
					if(!window.confirm(_showstr)){
						return;
					}
					//Calendar.getInstance().getEvent();
...
function bookNow(tid,username)
{
   ...
   $('#dialog1').dialog({
	...
	$('#rateButton12').click(function(){
					$('#dialog1').dialog('close');
					bookNow1(tid,username);
					});
			}
				else{bookNow1(tid,username);}
}	
		...
		if(window.returnvar == false)
		{
			...
			if(window.avl == false)
			{
				...
				if(window.firstBookNow == false){
					//var alertHTML = 'You are trying to book a session that conflicts with another booked timeslot. Please check My Bookings.';
