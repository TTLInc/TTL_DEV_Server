    	...
    	<div class="top-header">
            ...
            <div class="tutor-search">
            	<p>
                	...
                	<label><?php echo $llanguages;?></label>
                    <?php 
					echo form_dropdown('langInput1',$langsAll,@$sessionSearchData["langInput1Selected"],' id="langInput1" class="raduisSelect w78" ');?>
                </p>
                <p>
                	<label><?php echo $lkeyword;?></label>
                    <input name="keywords" id="keywords" type="text" class="raduisSelect" value="<?php echo @$sessionSearchData["keywordSelected"] ?>" placeholder="<?php echo $exp;?>">
                </p>
                <p>
                	<label>&nbsp;</label>
            ...
            <div class="icon-map serch-pop hide-map">
...
<div class="search_mid new-searchpage">
	<div class="search_rt_mid martop">
		<div class="result_title"><?php echo $lresults;?></div>
	...
	<dt>
		<a href="{profileUrl($T.uid)}/{$T.school_id}/{$T.school_id}"><img src="{profileImgResultThumb($T.pic)}" width="163" height="163"/></a>
			...
			<div class="fb-share-div">
				<div class="fb-share-button" data-href="{profileUrl($T.uid)}" data-layout="button"></div>
            </div>
			{#/if}
	</dt>
	<dd>
...
<?php endif; ?>
 ...
 <div id="dialog1" title="" style="display:None;">
			...
			<div class="ratelist">
				...
				<input  id="conversationcheck" checked value="0" type="radio" name="amex">  <?php echo  $ConvesationS;?> - <?php echo $InformalSpeaking ;?> <span id="Conv" id="conversationprise"  > </span> <?php  echo $CreditsTut;?><br><br>
 				</div>
	...
	*/		
			...
			var userprofileUrl = profileUrl(data.uid);
								...
								var infoHtml = "";
									...
									infoHtml += "	<p><span style='color:red; font-weight:bold;'>Rate: " + finalTutorRate.toFixed(2) + " credits/session</span></p>";
				...
				}else
				{
					...
					if (true || data.simple == "0"){
						...
						var infoHtml = "";
							...
							infoHtml += "	<p><span style='color:red; font-weight:bold;'>Rate: " + finalTutorRate.toFixed(2);
			...
			function pinOnSingleMap(data, i)
			{
					...
					geocoder.geocode( { 'address': ad}, function(results, status) {
						if (status == google.maps.GeocoderStatus.OK) 
						{
							...
							if (true || data.simple == "0"){
								...
								infoHtml += "credits1/session</span></p>";
								infoHtml += "	<div style='position:absolute; right:8px; top:0px;'><span class='small-grey-btn-wraper'><a class='small-grey-btn' href='"+profileUrl(data.uid) + ">View Profile</a></span></div>";
								infoHtml += "</div>";
								var infowindow = new google.maps.InfoWindow({
																				content: infoHtml,	
				...
				}else
				{
					...
					{
						//data.hRate = data.hRate * (1-(-window.configs['VEE_PRICE_PERCENT']['value']) );
						/*data.hRate = (data.hRate * window.configs['VEE_PRICE_PERCENT']['value']);
						data.hRate = Math.round(parseInt(data.hRate *10000) /100)  /100;*/
						var tutorRate = data.hRate;
						var TutorPercentageValue = tutorRate * window.configs['VEE_PRICE_PERCENT']['value'];
						var finalTutorRate = +tutorRate + +TutorPercentageValue;
