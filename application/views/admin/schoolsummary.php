	...
	<?php $j = 0; foreach ($payments as $key => $val) :?>
        <tr id="id_<?php echo $val['id'];?>">
			...
			<td id="actions" class="<?php echo $class; ?>" >
				...
				<?php } ?> -->
				<?php if($val['dues'] > 0){?>
				<a href="javascript:void(0);" class="appcat" mid="<?php echo $val['id']; ?>" val="<?php echo $val['dues']; ?>" schoolid="<?php echo $val['School_id']; ?>" schoolname="<?php echo $val['schoolname'];?>" tpayment="<?php echo number_format(@$val['tpayment'],2,'.',''); ?>" spayment="<?php if(@$val['spayment'] !='')echo @$val['spayment']; else echo '0.00'; ?>" title="Approve"><img alt="Approve" src="<?php echo base_url('/images/approve-red.png');?>" /></a>
				<?php } else {?>
...
function approve(_id,_val,_schoolid,_schoolname,_tpayment,_spayment)
{
	if(window.confirm('Are you sure you want to approve this payment?')){
		$.post('<?php echo Base_url("admin/AproveSchoolPayment")?>',{id:_id,approveval:_val,schoolid:_schoolid,schoolname:_schoolname,tpayment:_tpayment,spayment:_spayment},function(msg){			
		alert(msg);
			if (String == msg.constructor) {
				eval ('var json = ' + msg);
			} else {
			...
			if(json.status){
				...
				alert('Payment successfully done');
				 document.location.href="<?php echo base_url('admin/schoolsummary');?>";
			}
			else{
				alert(json.msg);
			}
...
$(function(){
	...
	$('.appcat').click(function(){
		approve([$(this).attr('mid')],[$(this).attr('val')],[$(this).attr('schoolid')],[$(this).attr('schoolname')],[$(this).attr('tpayment')],[$(this).attr('spayment')]);
	})
})
</script>
<script>
