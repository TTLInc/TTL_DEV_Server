        ...
        <tr>
		  ...
		  <?php if($sortorder == 'asc' && $sort == 'id'): ?>
           ...
           <?php if($sortorder == 'asc' && $sort == 'total'): ?>
			 ...
			 <th>Credit Date
			 <?php if($sortorder == 'asc' && $sort == 'pdate'): ?>
				<a href="<?php echo $currentUrl.'/sortorder/desc/sort/pdate'.'/limit/'.$limit.'/page/'.$page; ?>"><span class="css_right ui-icon ui-icon-triangle-1-n"></span></a>
				<?php else: ?>
				<a href="<?php echo $currentUrl.'/sortorder/asc/sort/pdate'.'/limit/'.$limit.'/page/'.$page; ?>"><span class="css_right ui-icon ui-icon-triangle-1-s"></span></a>
				<?php endif;?>
			 </th>
			 <th>Paid Date
			 <?php if($sortorder == 'asc' && $sort == 'pdate'): ?>
			...
			<th>Paid
			<?php if($sortorder == 'asc' && $sort == 'paid'): ?>
				<a href="<?php echo $currentUrl.'/sortorder/desc/sort/paid'.'/limit/'.$limit.'/page/'.$page; ?>"><span class="css_right ui-icon ui-icon-triangle-1-n"></span></a>
				<?php else: ?>
				<a href="<?php echo $currentUrl.'/sortorder/asc/sort/paid'.'/limit/'.$limit.'/page/'.$page; ?>"><span class="css_right ui-icon ui-icon-triangle-1-s"></span></a>
				<?php endif;?>
			</th>	
		   <th>Status</th>
            ...
            <?php 
            	...
            	//}else{
					...
					<a target="_blank" href="<?php echo base_url('admin/genpdf/id/'.$val['ref']);?>" class="genpdf" mid="<?php echo $val['ref']; ?>" title="Generate PDF"><img alt="Generate" src="<?php echo base_url('/images/pdf.png');?>"width="15px" /></a>
						...
						<a href="javascript:void(0);" class="appdel" mid="<?php echo $val['id']; ?>" val="<?php echo $val['delete']; ?>" title="Delete"><img alt="Delete" src="<?php echo base_url('/images/close-red.png');?>" /></a> 
						<?php }?>
...
setTimeout('showmenu("financial",7)',1000);
function del(_id){
...
function recdelete(_id)
{
	if(window.confirm('Are you sure you want to delete this payment record?')){
		$.post('<?php echo Base_url("admin/delaffiliate")?>',{id:_id},function(msg){
			if (String == msg.constructor) {      
				eval ('var json = ' + msg);
			} else {
				var json = msg;
			...
			if(json.status){
				//$('.appcat').attr('val',json.val);
				alert('Payment record is successfully deleted.');
				 document.location.href="<?php echo base_url('admin/referralpayment');?>";
			}
			else{
				alert(json.msg);
