...
<script type="text/javascript">
	...
	{ 
		...
		var res;
			...
			if(user.type =='t' && _studentConnected == '0' && _startTime == '-240')
			{ 
					$('.send_chat_ipt').val("Session Approved. We are sorry your student did not show up for this Free session, you may exit now.");
					jQuery('#chatSend').trigger('click');
					  $.post('<?php echo base_url("classroom/FreeBack");?>',{clsid:cid},function(result){
								});
			 
			} console.log('startTime'+_startTime);console.log('_studentConnected'+_studentConnected);
			if(user.type =='s' && _tutorConnected == '0' && _startTime == '-60')
		{
				 
				$('.send_chat_ipt').val("We are sorry your tutor did not show up. Please beep him a message or choose another wonderful tutor. Your credits will be reimbursed.");				
					...
					buttons: {
						...
						?>
							...
							$('.send_chat_ipt').val('Due to technical difficulties, '+ user['name'] +'has chosen to cancel this session within free period.This will not count as a paid session. Please Beep a message to'+ user['name'] +' if you want to clarify or reschedule. ');
								...
								$('.send_chat_ipt').val("The Student has decided to leave the session early. But don't worry, you will not  paid for this session.");
