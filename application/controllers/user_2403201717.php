	...
	*/
	...
	public function registerDo(){
		...
		if ($this->input->post()) {
			...
			$refid ='';
			$diffurl='';
			if($formData['refid'] == 'promosignup'){
				$formData['refid'] = '';
				$formData['reg_source'] = '1';
			}else{
				$formData['reg_source'] = '0';
			}
	...
	**/
	...
	public function forward_message($uid,$inbId,$subject){		
		...
		if($messageId != ''){
			...
			$str = $ffname.''.$flname.' sends this message to you from TheTalkList1:<br/><br/>';
			$str .= $message;
			$this->load->library('email');
			$this->email->mailtype = 'html';
			$this->email->from('no-reply@thetalklist.com',$fromName);
			$this->email->to($userEmail);
			$this->email->cc('beepbox@thetalklist.com');
			$this->email->subject($subject);
			$this->email->message($str);
			$this->email->send();
			return $userEmail;
...
/*if(count($result)>0){
		...
		$curr_stu = $this->user_model->getByUid($sid);
		if($curr_stu['is_eligible'] == 1 ){
			$cost = '0.00';	
		}
